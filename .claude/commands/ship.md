...
5b. **Public surface changed?** If the feature is user-visible (a new capability, tool, or tier change), update `README.md`, the landing `featureList` JSON-LD in `dashboard/index.html`, AND the manifest in `tools/check-copy-drift.js` in the SAME commit — the drift check gates CI. Keep canonical numbers (70 agents / 11 depts / 6 models / 4 tiers / pricing) untouched unless the auto-research facts are updated first. **The count appears in more phrasings than "N agents"** — sub-agents, pre-built/pre-configured/ready-to-use agents, named (AI) employees — so sweep on the NUMBER near an agent/employee word, not on one phrase, and check the carousel images too.
