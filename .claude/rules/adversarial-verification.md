...
4. **Verdict by majority — of a quorum.** Each skeptic returns `ship` or `block` plus findings scored against the rubric. High-risk: 2 of 3 must vote `ship`. A seat that errored, returned nothing, or returned no verdict word is **not a vote either way**, and a panel with fewer answered seats than a strict majority of its size is **inconclusive**: it blocks exactly as a `block` vote does, and is reported as inconclusive, never as `ship`. (The kernel's `adversarialVerify` enforces this — `answered`, `quorum`, `inconclusive` are on every result.) Any `block` vote's findings go back to the producing agent for revision — the deliverable re-enters the panel after revision (fresh skeptic contexts, same lenses).
