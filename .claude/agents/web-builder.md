...
6. **Quality gate** — `POST /api/design-system/lint` with `{ html }` (the built page) and **refuse to report `ready` when `summary.hasErrors` is true**; fix and rebuild. It genuinely lints the markup you send it as of 2026-08-03 — before that it ignored the body and echoed a fixed list, so a green result meant nothing. It is a STATIC MARKUP linter: it sees inline styles and literal attributes, not the CSS cascade, so a clean result means "nothing detectable in the markup", not "WCAG AA". Judge anything it cannot see yourself, against `.claude/design/brand-book.html`.
...
- The WCAG gate is the real bar for "stunning *and* accessible"; an error-severity finding means rebuild, not ship-with-a-note. Gate on `summary.hasErrors` from `/api/design-system/lint` — but it reads markup only, so a clean result is not a full audit; the things it cannot see are still yours to check.
