    ...
    <div class="mkt-queue-item">
      <div style="flex:1;min-width:0;">
        ...
        <div class="mkt-queue-meta">
          <span>${q.channel}</span>
          <span>${q.type}</span>
          ${q.generated ? '<span title="Generated by the marketing-hub agent from source content">&#10024; AI-atomized</span>' : ''}
          ${q.scheduledFor ? `<span>${new Date(q.scheduledFor).toLocaleString()}</span>` : ''}
        </div>
        ${q.content ? `<div style="font-size:12px;color:var(--text-secondary);margin-top:6px;white-space:pre-wrap;max-height:80px;overflow-y:auto;border-left:2px solid var(--border);padding-left:8px;">${escapeHtml(q.content)}</div>` : ''}
      </div>
...
async function showQueueContentModal() {
  showModal('Queue Content', `
    <div style="display:flex;flex-direction:column;gap:14px;">
      ...
      <div><label style="font-size:12px;color:var(--text-muted);display:block;margin-bottom:4px;">Source content (optional)</label>
        <textarea id="queueSourceContent" class="grok-input" rows="5" style="width:100%;resize:vertical;" placeholder="Paste the long-form content to atomize (blog post, transcript, etc.) — leave blank to just queue a placeholder title."></textarea>
        <div style="font-size:11px;color:var(--text-muted);margin-top:3px;">With source content, this calls the real marketing-hub agent to derive a platform-native post. Without it, this just queues the title as a placeholder.</div>
      </div>
      <button class="btn btn-primary" id="queueContentSubmitBtn" onclick="submitQueueContent()">&#128227; Queue Content</button>
    </div>
  `);
}
...
async function submitQueueContent() {
  ...
  const sourceContent = document.getElementById('queueSourceContent').value.trim();
  if (!title) return;
  const btn = document.getElementById('queueContentSubmitBtn');
  if (sourceContent && btn) { btn.disabled = true; btn.textContent = 'Atomizing…'; }
  const result = await fetchJSON('/api/marketing/queue', { method: 'POST', body: { title, channel, type, sourceContent: sourceContent || undefined } });
  if (result && result.error) {
    if (btn) { btn.disabled = false; btn.innerHTML = '&#128227; Queue Content'; }
    alert(result.error);
    return;
  }
  closeModal();
  loadMarketing();
  addTimelineEvent('marketing', `Content queued: ${title} → ${channel}${result && result.generated ? ' (AI-atomized)' : ''}`);
}
...
const settingsFieldMap = {
  ai: ['reasoning_mode', 'anthropic_api_key', 'openai_api_key', 'deepseek_api_key', 'zai_api_key', 'xai_api_key', 'gemini_api_key', 'perplexity_api_key', 'firecrawl_api_key', 'tavily_api_key', 'apify_api_token', 'manus_api_key', 'heygen_api_key', 'liveavatar_api_key', 'liveavatar_avatar_id', 'did_api_key', 'youtube_api_key', 'livekit_api_key', 'livekit_api_secret', 'livekit_url', 'deepgram_api_key', 'cartesia_api_key'],
  mcp: ['hermes_url', 'hermes_enabled'],
  email: ['provider', 'from_email', 'from_name', 'resend_api_key', 'smtp_host', 'smtp_port', 'smtp_user', 'smtp_pass', 'footer_address'],
  booking: ['slot_minutes', 'days_ahead', 'open_hour', 'close_hour', 'open_days'],
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
          <div style="background:var(--bg-card,#1e293b);border:1px solid var(--border,#334155);border-radius:10px;padding:20px;">
            ...
            <ul style="list-style:none;padding:0;margin:0 0 16px;font-size:12px;color:var(--text-muted,#94a3b8);line-height:1.9;">
              <li>&#9989; All 68 agents &amp; 11 departments</li>
