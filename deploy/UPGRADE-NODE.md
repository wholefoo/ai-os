# Runbook — Upgrade Node.js 20 → 22 on the VPS

**Why:** Node 20 (Iron) reached end-of-life on **2026-04-30** — no more security patches.
Node 22 (Jod) is Maintenance LTS through **2027-04-30**.
...
**rebuilt** against Node 22, or those processes will crash-loop on `NODE_MODULE_VERSION`
...
## 1. Install Node 22 (NodeSource, Debian)

```bash
curl -fsSL https://deb.nodesource.com/setup_22.x | sudo -E bash -
sudo apt-get install -y nodejs
node --version            # expect v22.x now
...
## 2. Point PM2's daemon at the new Node

PM2's in-memory daemon is still running under old Node until you refresh it:

```bash
sudo -u aios pm2 update    # respawns the PM2 daemon under Node 22, resurrecting processes
```
...
```bash
sudo npm install -g n8n         # reinstall rebuilds sqlite3 for Node 22
...
```bash
# Main server healthy under Node 22:
...
- The repo's `deploy/install-vps.sh` now pins `NODE_VERSION="22"`, so fresh installs land on
  the supported runtime — this runbook is only for the existing box.
- After a successful upgrade, the next nightly backup (3:30am) captures the working state.
- Consider Node 24 (Active LTS, through 2028) instead of 22 if you'd rather upgrade once and
  skip a cycle — substitute `setup_24.x` above. 22 is the more conservative, smaller jump.
