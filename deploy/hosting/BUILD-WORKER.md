...
Each build receives staged source and read-only runtime dependencies. Bubblewrap hides the application's files, credentials and host network. A transient systemd service limits memory (3 GB by default — set `AIOS_BUILD_MEMORY_MB` to change it), disables swap, limits processes to 128, limits CPU to one core and stops the whole process group at the deadline. The build's JS heap is derived from that cap, leaving roughly 1 GB for everything the heap does not cover: `/work` is a tmpfs, so the copy of `node_modules` and the `dist` output are charged to the same cgroup, and esbuild allocates outside V8 entirely. Set `AIOS_BUILD_HEAP_MB` only to override that derivation deliberately — a heap set close to the cap will be OOM-killed rather than failing cleanly. The limit was 768 MB with a fixed 512 MB heap, which left too little for the tmpfs and esbuild: content-heavy sites were killed, and ordinary ones were peaking within ~150 MB of the cap. Artifacts are validated before publication. A failed worker never falls back to unrestricted execution. See [Bubblewrap's isolation model](https://github.com/containers/bubblewrap).
