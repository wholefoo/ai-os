...
function newBudget(limits) {
  return {
    ...
    skippedLarge: [], skippedBinary: 0, denied: 0,
  };
}
...
const noteFor = (budget) => {
  ...
  if (budget.stopped) {
    const why = {
      ...
      // The RESULT cap, which used to be the one bound that did NOT announce itself: `walk` returns
      // early on a 'stop' from its callback without setting `budget.stopped`, so a search that found
      // exactly maxHits looked identical to one that had found them all. This module's own header
      // promises "every bounded result carries a note naming which budget ran out" — for this bound
      // it was not true. A capped list is the most dangerous kind of incomplete result, because it
      // is long enough to look exhaustive.
      hits: `at the ${LIMITS.maxHits}-result cap (there are almost certainly MORE)`,
    }[budget.stopped];
  ...
  if (budget.denied) {
    parts.push(`[... ${budget.denied} path(s) were EXCLUDED BY POLICY (secrets, instance state, the private`
      + ` commercial module) and were not read. Their absence from these results says nothing about`
      + ` whether they exist or what they contain — do not infer either. ...]`);
  }
  return parts.length ? '\n\n' + parts.join('\n') : '';
};

// Cap on brace expansion. `{a,b}{c,d}{e,f}...` multiplies, so an unbounded expansion is a cheap way
// to make the matcher do exponential work from a single tool argument.
const MAX_GLOB_ALTS = 64;

/**
 * Expand `{a,b,c}` alternations into a list of concrete patterns, innermost-first and nesting-aware.
 *
 * Braces were previously ESCAPED into literals by the regex builder, so `*.{js,ts,py}` compiled to a
 * pattern demanding a filename containing the characters `{js,ts,py}` — it could never match, and
 * returned a bare "No files matched." An unbalanced brace is left literal rather than throwing: a
 * caller who genuinely wants a `{` in a filename should still be able to search for it.
 */
function expandBraces(pattern, cap = MAX_GLOB_ALTS) {
  ...
  for (const ch of pattern.slice(open + 1, close)) {
    ...
    else if (ch === ',' && d === 0) { opts.push(cur); cur = ''; }
    else cur += ch;
  }
  opts.push(cur);
  ...
  const out = [];
  for (const o of opts) {
    for (const p of expandBraces(pre + o + post, cap)) {
      out.push(p);
...
/**
 * Compile one brace-free glob to a RegExp, SEGMENT BY SEGMENT.
 *
 * Segment-wise rather than by string substitution, because the old string approach mapped `**` to
 * `.*` and left the following `/` in place. That made `**\/*.js` compile to `^.*\/[^/]*\.js$`, which
 * REQUIRES a slash — so it matched 177 files while silently omitting the two that live at the repo
 * root, including `server.js`, the 716KB monolith that most of this codebase lives in. An auditor
 * asking for "every .js file" got a confident list with the most important file missing and nothing
 * to indicate it. `**` must match ZERO OR MORE segments, which is only expressible per-segment.
 *
 * `?` was not handled at all: it fell through to the regex as a quantifier, so `serve?.js` quietly
 * meant "serv, optional e" instead of "one character". Same silent-wrong-answer class.
 */
function globToRegExp(pattern) {
  const escape = (s) => s.replace(/[.+^${}()|[\]\\]/g, '\\$&');
  const seg = (s) => s.split('').map((ch) => (ch === '*' ? '[^/]*' : ch === '?' ? '[^/]' : escape(ch))).join('');
  ...
  parts.forEach((p, i) => {
    ...
    if (p === '**') {
      // Trailing `**` matches everything below; otherwise zero-or-more whole segments — the `(?:…)*`
      // is what lets `**/x` also match a root-level `x`.
      src += last ? '.*' : '(?:[^/]*/)*';
      return;
    }
    src += seg(p) + (last ? '' : '/');
...
function createRepoTools({ base, isPathAllowed, limits = LIMITS } = {}) {
  ...
  function canReadDiskPath(abs) {
    try {
      const real = fs.realpathSync(abs);
      return (real === realBase || real.startsWith(realBase + path.sep))
        && isPathAllowed(path.relative(realBase, real).replace(/\\/g, '/'));
    } catch { return false; }
  }
  ...
  function walk(startDir, onFile, budget) {
    ...
    while (stack.length) {
      ...
      for (const e of entries) {
        ...
        const r = rel(abs);
        // A denied file must be COUNTED, not just skipped. Silently omitting it makes "not permitted"
        // read as "not present" — which is precisely how a security-auditor concluded this repo had
        // no lockfile (DEP-01, run-1786080073868) when package-lock.json has been committed since
        // July: it was denied, Glob returned nothing, and absence became a HIGH finding.
        if (!isPathAllowed(r)) { budget.denied++; continue; }
  ...
  function readTextFile(abs, budget) {
    if (!canReadDiskPath(abs)) { budget.denied++; return null; }
  ...
  function grep(args, budget) {
    ...
    const hits = [];
    const scan = (abs, r) => {
      if (hits.length >= limits.maxHits) { budget.stopped = 'hits'; return 'stop'; }
    ...
    if (args.path) {
      ...
      if (!canReadDiskPath(abs)) return 'Error: path escapes the repo root or is denied.';
      let st;
  ...
  function glob(args, budget) {
    ...
    try {
      ...
      res = alts.map(globToRegExp);
    } catch (e) {
      return `Error: invalid glob pattern — ${(e && e.message) || e}`;
    }
    const matches = [];
    walk(base, (abs, r) => {
      if (matches.length >= limits.maxHits) { budget.stopped = 'hits'; return 'stop'; }
      if (res.some((re) => re.test(r))) matches.push(r);
      return undefined;
    }, budget);
    if (matches.length) return matches.join('\n') + noteFor(budget);

    // A ZERO-MATCH RESULT MUST EXPLAIN ITSELF. This is the whole point of the rewrite:
    // run-1786158988267's architecture stage read a bare "No files matched." from the pattern
    // `*.{js,ts,py}` (braces were not supported, so it could never match), saw the unrelated
    // EXCLUDED-BY-POLICY note in the same response, and concluded that all application source was
    // denied by policy. It then declared the entire stage NOT ASSESSABLE. Two independent facts read
    // as one, because a zero was indistinguishable from a refusal. Saying how the pattern was
    // actually interpreted lets the caller see a misunderstanding instead of inferring a conspiracy.
    const how = alts.length > 1
      ? `interpreted as ${alts.length} alternatives: ${alts.slice(0, 8).join(', ')}${alts.length > 8 ? ', …' : ''}`
      : `interpreted as: ${alts[0]}`;
    return 'No files matched.'
      + `\n\n[... The pattern was ${how}. Reminders: \`*\` and \`?\` do NOT cross \`/\`, so \`*.js\``
      + ` matches only the repo ROOT — use \`**/*.js\` for every depth (it includes the root too).`
      + ` This means "the pattern matched nothing", which is NOT the same as "no such files exist"`
      + ` and NOT the same as "they were withheld" — any withheld paths are reported separately below. ...]`
      + noteFor(budget);
  }
