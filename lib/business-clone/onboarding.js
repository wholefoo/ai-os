...
/**
 * The persona to JUDGE a clone by — the effective one (personal + inherited company policy) when
 * the caller can supply it, the clone's own otherwise.
 *
 * This module is pure and knows nothing about org profiles, so every readiness question here used
 * to read `c.persona` directly. For anyone whose company answers part of their identity — which is
 * every employee, and the founder too once a company profile exists — that persona is legitimately
 * incomplete on its own. The consequences were all of the "the software is wrong about me" kind:
 * onboarding never completed, and the dashboard nudged them to finish an interview that had
 * nothing left to ask.
 *
 * Takes a RESOLVER rather than a list of pre-computed personas: these functions receive a list of
 * clones, and pairing two arrays by index is a bug waiting for the first caller who filters one.
 * Defaults to the clone's own persona, so a solo owner and every existing caller are unaffected.
 */
function personaOf(clone, effectiveOf) {
  if (!clone) return null;
  return (typeof effectiveOf === 'function' ? effectiveOf(clone) : null) || clone.persona;
}
 ...
 */
function reconcile(record, clones, effectiveOf) {
  const usable = (clones || []).some((c) => c && persona.isUsable(personaOf(c, effectiveOf)).usable);
 ...
 */
function shouldPrompt(record, clones, effectiveOf) {
  if (!record) return true;
  if (record.status === 'dismissed' || record.status === 'completed') return false;
  return !(clones || []).some((c) => c && persona.isUsable(personaOf(c, effectiveOf)).usable);
}
...
function overview(record, clones, effectiveOf) {
  const list = clones || [];
  const best = list.reduce((acc, c) => Math.max(acc, persona.completeness(personaOf(c, effectiveOf)).overall), 0);
  return {
    status: record ? record.status : 'pending',
    disclosureAccepted: disclosureAccepted(record),
    disclosureVersion: DISCLOSURE_VERSION,
    shouldPrompt: shouldPrompt(record, list, effectiveOf),
