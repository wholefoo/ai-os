 ...
 */
function screenDispatch(personaInput, { agent, task, context = '', companyBoundaries = null } = {}) {
  ...
  // Same wording rule as the inbound screen: an employee did not choose the company's limits, so do
  // not tell them they did.
  for (const topic of p.boundaries.requiresHuman) {
    if (!persona.mentions(body, topic)) continue;
    boundaryBlocked = true;
    reasons.push(persona.limitSource(topic, cb.requiresHuman) === 'company'
      ? `This is about "${topic}", which your company reserves for a person — so your clone will not commission work on it either.`
      : `This is about "${topic}", which you asked to handle personally — so your clone will not commission work on it either.`);
  }
  for (const topic of p.boundaries.confidentialTopics) {
    if (!persona.mentions(body, topic)) continue;
    boundaryBlocked = true;
    reasons.push(persona.limitSource(topic, cb.confidentialTopics) === 'company'
      ? `This touches "${topic}", which your company marked confidential — it does not get handed to another agent.`
      : `This touches "${topic}", which you marked confidential — it does not get handed to another agent.`);
  }
 ...
 */
function validateSelection(parsed, personaInput, { goal, context = '', companyBoundaries = null } = {}) {
  ...
  const screen = screenDispatch(personaInput, { agent, task, context, companyBoundaries });
