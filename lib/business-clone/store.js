...
//  This platform has no workspace primitive today — core state is flat and single-instance. This
//  feature is nonetheless specified as a per-client, sellable capability, so every clone carries a
//  `clientId` from the very first record written, and EVERY read path in this module takes a
//  clientId and filters by it. On a single-operator instance that id is simply a constant
//  (OPERATOR_CLIENT_ID). The point is that when a real workspace layer lands, this module already
//  behaves correctly and the migration is "populate a different id", not "retrofit isolation into
//  a store that never had it" — which is the version of this that leaks one customer's voice
//  replica into another customer's drafts.
//
//  Because that isolation is currently enforced HERE and not by a database boundary, the filtering
//  is deliberately unconditional: there is no "list all clones" export, and getClone requires the
//  clientId to match even when you already hold the clone's unique id. A caller that forgets to
//  scope gets nothing back rather than everything.
//
...
// The clientId used on a single-operator instance, where "the client" is the operator themselves.
// Not a magic string scattered through callers — import it.
...
function cleanId(v) {
  // Ids appear in log lines and (later) file paths; keep them boring.
 ...
 */
function createClone({ id, clientId, name, ownerName = '' }) {
