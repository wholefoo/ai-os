...
function screenInbound(personaInput, inboundText) {
  ...
  for (const topic of p.boundaries.requiresHuman) {
    if (persona.mentions(lower, topic)) reasons.push(`This mentions "${topic}", which you asked to handle personally.`);
  }
  for (const topic of p.boundaries.confidentialTopics) {
    if (persona.mentions(lower, topic)) reasons.push(`This touches "${topic}", which you marked confidential.`);
  }
