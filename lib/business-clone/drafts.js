...
/** Case-insensitive whole-ish phrase presence, matching persona.checkRedLines' matching rule. */
function mentions(haystackLower, phrase) {
  return haystackLower.includes(String(phrase).toLowerCase());
}
 ...
 */
function screenInbound(personaInput, inboundText) {
  ...
  for (const topic of p.boundaries.requiresHuman) {
    if (mentions(lower, topic)) reasons.push(`This mentions "${topic}", which you asked to handle personally.`);
  }
  for (const topic of p.boundaries.confidentialTopics) {
    if (mentions(lower, topic)) reasons.push(`This touches "${topic}", which you marked confidential.`);
  }
