...
// Deduping is not cosmetic here: every list field is compiled into a system prompt on every clone
// call, so a repeated entry costs tokens forever and adds nothing. `str` is the admissibility rule —
// strings only; a number or an object in a voice field is bad input, not content to be stringified.
function strList(v, cap = CAPS.shortText, maxItems = CAPS.listItems) {
  return cappedList(v, { cap, maxItems, coerce: str });
}
