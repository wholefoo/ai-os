...
function isUsable(persona) {
  ...
  if (!p.identity.ownerName || !p.identity.whatTheyDo) {
    reasons.push('identity is missing the owner name or a description of what the business does');
  }
...
function mentions(lowerBody, phrase) {
  return new RegExp(`\\b${escapeRe(phrase.toLowerCase())}`, 'i').test(lowerBody);
}
