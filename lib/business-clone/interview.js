 ...
 */
function buildAskPrompt(clone, effectivePersona, knownFromCompany = []) {
  const templateId = clone.templateId || DEFAULT_TEMPLATE;
  // Ask against the EFFECTIVE persona. For an employee, the company has already answered what the
  // business is called, what industry it is in and what it does — asking them again would waste
  // their time and collect five different answers to the same question across five people.
  const known = effectivePersona || clone.persona;
  const target = nextDimension(known, templateId);
  if (!target) return null; // interview is done

  const seeds = seedQuestions(target.dimension, target.missing, templateId);
  const ownerName = known.identity.ownerName || 'the owner';
  ...
  const task = [
    ...
    '',
    // Skipping the dimension is not enough on its own: a model given the company's name in context
    // will cheerfully open with "so tell me about the business". Naming what is already settled is
    // what actually stops it being re-asked. Spread so the blank-line separators around it survive.
    ...(knownFromCompany.length
      ? ['', `Already on file from the company — do NOT ask about any of these: ${knownFromCompany.join(', ')}.`]
      : []),
    '',
    'Conversation so far:',
  ...
  return { system, task, dimension: target.dimension, seeds, knownFromCompany };
}
