...
async function getText(url, { headers = {}, timeoutMs = 10000 } = {}) {
  const res = await fetch(url, { headers: { 'User-Agent': UA, Accept: '*/*', ...headers }, redirect: 'follow', signal: AbortSignal.timeout(timeoutMs) });
...
async function srcGoogleTrends(topic, { geo = 'US' } = {}) {
  ...
  if (topic) items = items.filter((i) => i.title.toLowerCase().includes(topic.toLowerCase()));
