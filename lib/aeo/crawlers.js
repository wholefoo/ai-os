...
async function checkAiCrawlers(domain) {
  ...
  try {
    const res = await fetch(`https://${host}/robots.txt`, { headers: { 'User-Agent': UA }, redirect: 'follow', signal: AbortSignal.timeout(10000) });
    if (res.ok) { robots = await res.text(); accessible = true; }
  } catch { /* no robots / unreachable */ }
...
module.exports = { AI_CRAWLERS, parseBlocks, isBlocked, checkAiCrawlers, generateRobotsAllowlist };
