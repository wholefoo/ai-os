...
async function reflexionAttempt(task, d, budget, memory, opts, attempt) {
  ...
  const act = await guardedCall(d, budget, opts.actor || 'coder', actPrompt,
    { maxTokens: opts.maxTokens, context: opts.context });
  ...
  // The evaluator is `reviewer` (effort: xhigh). At 800 it returned ""; at 2000 it returned the
  // 8 characters `VERDICT:`. The ceiling now comes from the budget.js floor (executeAgent's own
  // default), and the API's exact stop_reason says if even that is too small.
  const ev = await guardedCall(d, budget, opts.evaluator || 'reviewer', evalPrompt, { maxTokens: opts.evalTokens });
  if (!ev.ok) {
  ...
  // One sentence out, but the reflector may be an xhigh agent. Ceiling from the budget.js floor —
  // this site was raised by hand twice (300 -> 1200 -> 1500) and was still wrong; the floor ended that.
  const rf = await guardedCall(d, budget, opts.reflector || opts.actor || 'coder', reflectPrompt, {});
