...
async function reflexionAttempt(task, d, budget, memory, opts, attempt) {
  ...
  const ev = await guardedCall(d, budget, opts.evaluator || 'reviewer', evalPrompt, { maxTokens: opts.evalTokens || 800 });
  if (!ev.ok) {
  ...
  const rf = await guardedCall(d, budget, opts.reflector || opts.actor || 'coder', reflectPrompt, { maxTokens: 1200 });
