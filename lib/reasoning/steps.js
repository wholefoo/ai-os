 ...
 *
 * Delegates to models.parseList, which is shared with tot.parseThoughts so the two can never drift
 * apart in what they accept — see that function's header for the live failure that widened it
 * (a real Opus answered `**Step 1 — ...`, which the old pattern matched not at all).
 */
function parseSteps(text, max = MAX_STEPS) {
  const out = M.parseList(text, max);
  // A model that ignored the format entirely still gave us prose worth one step. REACHING THIS
  // BRANCH IS ITSELF A SIGNAL: it fired on a perfectly good three-step plan and silently collapsed
  // it to one, because the parser was too strict rather than because the model was unstructured.
  if (!out.length) {
...
// `maxTokens` defaults to null so the budget.js floor applies. This site once passed 400 and got
// back a literal "" from an xhigh reviewer — the reply is three short lines, but adaptive thinking
// spends from the same ceiling, and 400 was gone before the first line.
function modelVerifier({ agent = 'reviewer', passScore = DEFAULT_PASS_SCORE, maxTokens = null } = {}) {
...
async function decompose(task, deps, { agent = 'architect', maxSteps = 6, budget, maxTokens = null, context = '' } = {}) {
  const prompt = [
...
async function runVerifiedSteps(task, depsIn, opts = {}) {
  ...
  for (let i = 0; i < plan.steps.length; i++) {
    ...
    for (let attempt = 0; attempt <= maxRevisions; attempt++) {
      ...
      const act = await guardedCall(d, budget, actor, actPrompt, { maxTokens: opts.maxTokens, context: opts.context });
      if (!act.ok) {
