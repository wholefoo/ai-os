...
async function proposeThoughts(task, pathText, d, budget, opts) {
  ...
  // 2500, not 1000. The proposer is `architect` (effort: xhigh) and it must emit N DISTINCT
  // thoughts — the most output-heavy call in the framework, against the same ceiling its adaptive
  // thinking spends from. At 500 the live check got one thought where it asked for two.
  const r = await guardedCall(d, budget, opts.proposer || 'architect', prompt, { maxTokens: opts.proposeTokens || 2500 });
...
async function scoreThought(task, pathText, thought, d, budget, opts) {
  ...
  // Same xhigh + adaptive-thinking headroom as the PRM verifier: the answer is two lines, the budget is for the thinking.
  const r = await guardedCall(d, budget, opts.scorer || 'reviewer', prompt, { maxTokens: opts.scoreTokens || 1500 });
...
async function voteThoughts(task, pathText, thoughts, d, budget, opts) {
  ...
  // 200 here handed an xhigh reviewer a ceiling it could essentially never answer within. Found by
  // sweeping every call site rather than by it failing — no live run had exercised `vote` scoring.
  const r = await guardedCall(d, budget, opts.scorer || 'reviewer', prompt, { maxTokens: 1500 });
