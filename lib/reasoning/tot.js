...
function parseThoughts(text, k) {
  ...
  const out = [];
  const START = /^\s*(?:thought\s*)?(?:\d+[.)]|[-*•])\s+(.*)$/i;
  for (const line of raw.split('\n')) {
    const m = line.match(START);
    if (m) { if (m[1].trim()) out.push(m[1].trim()); }
...
async function scoreThought(task, pathText, thought, d, budget, opts) {
  ...
  const r = await guardedCall(d, budget, opts.scorer || 'reviewer', prompt, { maxTokens: opts.scoreTokens || 300 });
