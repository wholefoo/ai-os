...
/**
 * Is this line the START of a numbered/bulleted item? Returns its text, or null.
 *
 * ── WHY THIS IS SHARED AND WHY IT IS THIS TOLERANT ──
 * steps.js and tot.js each had their own copy of a stricter regex, and the LIVE check
 * (tools/verify-reasoning-live.js, 2026-09-02) caught what mocks never could: asked for numbered
 * steps, a real Opus replied
 *
 *     **Step 1 — Establish the actual code path of the nightly export job...
 *
 * — markdown-bold, an em-dash instead of a period. The old pattern required `1.` / `1)` / a bare
 * bullet, so it matched NOTHING, fell through to the "model ignored the format" fallback, and
 * returned the entire reply as ONE step. A three-step plan silently became a one-step plan; nothing
 * errored. Models format lists in markdown because that is how they are trained to write, and a
 * parser that only accepts the literal shape asked for will keep losing that argument.
 *
 * Deliberately NOT loose enough to match prose: an item must carry a real marker — a bullet, or a
 * number followed by a delimiter — so an ordinary sentence still cannot become a step.
 */
function listItemStart(line) {
  const t = String(line || '').replace(/^\s+/, '');
  // Strip a DANGLING emphasis marker the opening `**` left behind: `**Step 1 — do the thing**`
  // closes its bold after the text, so the item would otherwise carry a literal `**` into prompts
  // and into saved STaR traces.
  const clean = (s) => s.trim().replace(/(\*\*|__)+\s*$/, '').trim();

  // A bullet: - • or a SINGLE * (`**` is bold emphasis opening a numbered item, handled below).
  let m = t.match(/^(?:[-•]|\*(?!\*))\s+(.+)$/);
  if (m) return clean(m[1]);
  // A number, optionally wrapped in markdown emphasis/heading and optionally prefixed by a word
  // like "Step"/"Thought", followed by . ) : — – or -
  m = t.match(/^[#>\s]*(?:\*\*|__)?\s*(?:step|thought)?\s*\d+\s*(?:\*\*|__)?\s*[.):—–-]\s*(.+)$/i);
  if (m) return clean(m[1]);
  return null;
}

/**
 * Parse a numbered/bulleted reply into items, joining wrapped continuation lines onto their item.
 * Shared by steps.parseSteps and tot.parseThoughts so the tolerance can never drift between them.
 */
function parseList(text, max = 12) {
  const out = [];
...
module.exports = {
  ...
  // listItemStart stays internal — parseList is the only caller and the only thing worth a consumer.
  parseList,
};
