...
// Canonical pipeline stages (funnel order; churned is terminal/manual — see sync.js STAGE_RANK).
// The kanban board renders exactly these columns, so writes must stay inside the set — an
// arbitrary string would silently create a phantom column the UI can't show.
const STAGES = ['lead', 'audited', 'onboarding', 'customer', 'churned'];
...
function registerCrmRoutes(app, ctx) {
  ...
  app.get('/api/crm/stats', requireAdmin, (req, res) => {
    res.json({ ...repo.contacts.stats(), stages: STAGES });
  });

  // Pipeline board: contacts grouped by stage in one call (capped per column; column headers
  // show the true byStage totals from /stats even when a column is truncated).
  app.get('/api/crm/pipeline', requireAdmin, (req, res) => {
    const per = Math.min(Number(req.query.per) || 30, 100);
    const columns = {};
    for (const s of STAGES) columns[s] = repo.contacts.page({ stage: s, limit: per, sort: 'updated' }).rows || [];
    res.json({ stages: STAGES, columns, byStage: repo.contacts.stats().byStage });
  });
  ...
  app.post('/api/crm/contacts', requireAdmin, (req, res) => {
    ...
    if (stage !== undefined && !STAGES.includes(stage)) return res.status(400).json({ error: `stage must be one of: ${STAGES.join(', ')}` });
  ...
  app.patch('/api/crm/contacts/:id', requireAdmin, (req, res) => {
    ...
    if (req.body && req.body.stage !== undefined && !STAGES.includes(req.body.stage)) {
