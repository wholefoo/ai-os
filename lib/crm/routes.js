...
function registerCrmRoutes(app, ctx) {
  const { requireAdmin, webStudioSites, brandKits, broadcast = () => {} } = ctx;
  ...
  app.get('/api/crm/contacts/:id', requireAdmin, (req, res) => {
    ...
    res.json({ contact, activities, sites: linkedSites, brandKits: ownedKits });
  });
  ...
  app.patch('/api/crm/contacts/:id', requireAdmin, (req, res) => {
    ...
    if (body.stage && body.stage !== before.stage) {
      repo.activities.add({ contactId: before.id, type: 'stage_change', body: `Stage: ${before.stage} → ${body.stage}`, author: body.author || 'admin' });
    }
    bump({ id: before.id });
  ...
  app.post('/api/crm/contacts/:id/notes', requireAdmin, (req, res) => {
    ...
    repo.activities.add({ contactId: c.id, type: 'note', body: text, author: (req.body || {}).author || 'admin' });
    bump({ id: c.id });
