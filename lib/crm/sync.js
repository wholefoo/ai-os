...
function syncFromUsers(users = []) {
  ...
  for (const u of users) {
    ...
    repo.contacts.upsertByEmail({
      ...
      support_expires_at: u.supportExpiresAt || undefined,
...
function syncFromSites(webStudioSites = []) {
  // Sites have NO owner field, so we can only auto-match by domain; everything else
  // surfaces in the dashboard "unassigned sites" worklist for manual linking.
  let linked = 0;
  for (const site of webStudioSites) {
    if (!site || !site.id || !site.domain) continue;
    const c = repo.contacts.findByDomain(site.domain);
    if (c) { repo.links.add({ contactId: c.id, siteId: site.id, domain: site.domain }); linked++; }
  }
  return linked;
}
