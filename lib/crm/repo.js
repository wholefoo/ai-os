...
const contacts = {
  ...
  /**
   * Erasure (GDPR Art. 17 / CCPA): the contact row and everything keyed to it. The child deletes
   * are EXPLICIT because this connection never turns `PRAGMA foreign_keys` on, so the schema's
   * `ON DELETE CASCADE` is decorative — deleting the contact alone would orphan its activities
   * (which carry the lead's own words) and defeat the erasure. Explicit BEGIN/COMMIT: node:sqlite
   * has no transaction helper. A contact that does not exist erases nothing and says so.
   */
  eraseByEmail(email) {
    const e = normEmail(email);
    if (!e) return { erased: false, activities: 0, siteLinks: 0 };
    const db = getDb();
    const row = db.prepare('SELECT id FROM contacts WHERE tenant_id = ? AND email = ?').get('master', e);
    if (!row) return { erased: false, activities: 0, siteLinks: 0 };
    db.exec('BEGIN');
    try {
      const activities = db.prepare('DELETE FROM activities WHERE contact_id = ?').run(row.id).changes;
      const siteLinks = db.prepare('DELETE FROM site_links WHERE contact_id = ?').run(row.id).changes;
      db.prepare('DELETE FROM contacts WHERE id = ?').run(row.id);
      db.exec('COMMIT');
      return { erased: true, activities, siteLinks };
    } catch (err) {
      db.exec('ROLLBACK');
      throw err;
    }
