...
function resolveSafeWorkspace(ws) {
  ...
  if (!ok) throw new Error('workspace is outside the allowed roots');
...
function run(args, { cwd, timeoutMs } = {}) {
  return new Promise((resolve) => {
    ...
    let child;
    try {
      child = spawn(CFG.bin, args, { cwd, env, windowsHide: true });
...
async function audit({ workspace, deps } = {}) {
  ...
  try {
    ...
    if (r.code === -1) return { ok: false, available: false, error: r.err.slice(0, 300) };
    return { ok: true, exitCode: r.code, out: r.out.slice(-4000), reportsDir: outBase };
  } finally { /* keep reports dir for the caller; cleaned by retention */ }
}
...
module.exports = { configure, isEnabled, doctor, threatModel, assess, audit, resolveSafeWorkspace };
