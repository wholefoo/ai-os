...
async function semgrepScan(dir, { config } = {}) {
  ...
  const r = await runBin(CFG.semgrepBin || 'semgrep', ['--config', cfg, '--json', '--quiet', '--timeout', '120', '--exclude', 'node_modules', '--exclude', '.git', real], { timeoutMs: 5 * 60 * 1000 });
  ...
  // Honesty guard: a "blind" run — rules failed to load (registry fetch error) or nothing was
  // actually scanned — must NOT report clean. Surface it as unavailable so the publish gate fails
  // OPEN (doesn't block) rather than falsely passing an unscanned site. A real 0-finding scan
  // (rules loaded, files scanned, no errors) still returns available:true.
  const errs = Array.isArray(parsed.errors) ? parsed.errors : [];
  const scanned = (parsed.paths && Array.isArray(parsed.paths.scanned)) ? parsed.paths.scanned : null;
  if (!parsed.results.length && (errs.length || (scanned && scanned.length === 0))) {
    return { available: false, reason: errs.length ? `semgrep reported ${errs.length} error(s) — rules may not have loaded` : 'semgrep scanned no files', detail: JSON.stringify(errs.slice(0, 2)).slice(0, 300) };
  }
