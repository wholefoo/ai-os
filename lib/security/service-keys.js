...
const SCOPES = Object.freeze({
  ...
  content: 'Web Studio content only — articles, ingest and rebuilds — optionally bound to one site',
  admin: 'everything the static API token can do (never a human decision)',
});

// What a `content` key may reach, in EVERY method, reads included. The other scopes may GET
// anything; a publishing workflow has no business reading the CRM, settings or other keys, so this
// one is confined to content routes for reads too. Group 1 is the site id, checked against the
// key's binding. Paths, not prefixes: a new Web Studio route is closed to content keys until it is
// added here on purpose.
const CONTENT_ALLOW = Object.freeze([
  /^\/api\/web-studio\/sites\/([^/?#]+)\/articles(?:\/[^/?#]+)?$/,
  /^\/api\/web-studio\/sites\/([^/?#]+)\/ingest$/,
  /^\/api\/web-studio\/sites\/([^/?#]+)\/build$/,
]);
const SITE_ID = /^[A-Za-z0-9-]{1,64}$/;
const MAX_LABEL = 80;
...
/** Validate creation input. Returns { error } or { label, scope, expiresAt, siteId }. */
function validateNew({ label, scope, expiresInDays, siteId } = {}, now = Date.now()) {
  ...
  // A site binding only means something for a content key. Refused on the others rather than
  // silently stored, so nobody believes an admin key is confined to one site when it is not.
  let bound = null;
  if (siteId !== undefined && siteId !== null && siteId !== '') {
    if (scope !== 'content') return { error: 'siteId applies only to the "content" scope' };
    if (typeof siteId !== 'string' || !SITE_ID.test(siteId)) return { error: 'siteId is not a valid site id' };
    bound = siteId;
  }
  return { label: label.trim(), scope, expiresAt, siteId: bound };
}

/** Build a key record from validated input. Returns { key, token } — the token leaves this function once. */
function createKey({ id, label, scope, expiresAt, createdBy, siteId }, now = Date.now()) {
  const token = mintToken();
  const key = { id, label, scope, siteId: siteId || null, tokenHash: sha256(token), createdAt: new Date(now).toISOString(), createdBy: createdBy || null, expiresAt: expiresAt || null, lastUsedAt: null, revoked: false, revokedAt: null, rotatedFrom: null };
  return { key, token };
}
...
function decideScope(key, method, path) {
  ...
  // BEFORE the general read allowance: a content key is confined for reads too. A service key
  // resolves to an admin principal, so wsFindSite's ownership check does not restrict it — the URL
  // path is the only place the site binding CAN be enforced, and it is enforced here.
  if (key.scope === 'content') {
    const hit = CONTENT_ALLOW.map((re) => p.match(re)).find(Boolean);
    if (!hit) return { allow: false, reason: `scope "content" may not ${m} ${p}` };
    let site = hit[1];
    try { site = decodeURIComponent(site); } catch { return { allow: false, reason: 'malformed site id' }; }
    if (key.siteId && site !== key.siteId) {
      return { allow: false, reason: 'this content key is bound to a different site' };
    }
    return { allow: true };
  }
...
function publicView(k) {
  return { id: k.id, label: k.label, scope: k.scope, siteId: k.siteId || null, createdAt: k.createdAt, createdBy: k.createdBy, expiresAt: k.expiresAt, lastUsedAt: k.lastUsedAt, revoked: !!k.revoked, revokedAt: k.revokedAt || null, rotatedFrom: k.rotatedFrom || null };
}
