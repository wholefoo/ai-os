...
//
// No native "extend" support here on purpose: it was built, live-tested, and pulled 2026-07-20
// after four straight rounds of live failures against Google's actual endpoint (numberOfVideos
// rejected, inlineData rejected, a generic "internal server issue," then an open/unresolved
// "must be a video that was generated by VEO that has been processed" bug still active on
// Google's own developer forum with no confirmed fix). Independent clips stitched with hard cuts
// (concatVideos below) don't depend on any of that chain-continuity machinery and are what most
// real multi-scene video content actually is edited as anyway.
...
async function submitGeneration({ apiKey, prompt, tier, resolution, durationSeconds, aspectRatio }) {
  ...
  if (!res.ok) throw httpError(res, await res.json().catch(() => ({})), 'Veo');
...
async function pollOperation({ apiKey, operationName }) {
  ...
  if (!res.ok) throw httpError(res, await res.json().catch(() => ({})), 'Veo poll');
  return res.json();
}
...
// Verified against real ffmpeg 2026-07-19 (synthetic clips, ffprobe-confirmed the output duration
// is genuinely the sum of its inputs, not a truncated or single-clip copy).
