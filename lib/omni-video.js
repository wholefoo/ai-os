  ...
  const res = await fetch(`${API_BASE}/models/${t.model}:predictLongRunning?key=${apiKey}`, {
    ...
    body: JSON.stringify({
      ...
      parameters: {
        ...
        // Verified against the real endpoint 2026-07-18.
...
// Stitch multiple same-tier/resolution clips (every scene in one production shares both, so their
// streams are compatible) into one file via ffmpeg's concat demuxer with stream copy — no
// re-encoding, fast and lossless. ffmpeg is already a dependency on this server (see server.js's
// YouTube frame-extraction step); this reuses the same execFileSync(array-args) pattern, which
// never goes through a shell, so there is no injection surface even though input paths could in
// principle vary — in practice every caller here passes only paths this module itself generated.
function concatVideos({ inputPaths, destPath }) {
  if (!inputPaths || inputPaths.length < 2) throw new Error('concatVideos needs at least 2 input files');
  const listFile = path.join(path.dirname(destPath), `concat-${crypto.randomUUID()}.txt`);
  const listContent = inputPaths.map((p) => `file '${p.replace(/'/g, "'\\''")}'`).join('\n');
  fs.writeFileSync(listFile, listContent);
  try {
    execFileSync('ffmpeg', ['-f', 'concat', '-safe', '0', '-i', listFile, '-c', 'copy', destPath, '-y'],
      { timeout: 120000, stdio: ['ignore', 'ignore', 'pipe'] });
  } catch (e) {
    throw new Error(`ffmpeg concat failed: ${(e.stderr || e.message || '').toString().slice(0, 500)}`);
  } finally {
...
module.exports = {
  ...
  submitGeneration, pollOperation, downloadVideo, generateVideo, concatVideos,
};
