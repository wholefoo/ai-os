...
function parseFrontmatter(fm) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    //     - file-write
    //     - content-creation
    // 23 of the 68 agents use this form and 33 use the inline one. Reading only the inline form made
    // this module report those 23 as having NO tools, which under-counted key 4 (access to tools and
    // files) by a third of the corpus — a wrong number in the report that plans the next phase. The
    // parser has to match the files that exist, not the ones that would be tidier.
    if (value === '') {
      const items = [];
      let j = i + 1;
...
function sectionBullets(body, heading) {
  ...
  for (let i = start + 1; i < lines.length; i++) {
    ...
    if (/^\s+\S/.test(line)) continue;                // wrapped continuation of a bullet
