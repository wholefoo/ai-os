...
/**
 * Ceiling on the MERGED list. P3 stacks a skill brief's criteria over an agent's handbook rubric,
 * which already carries its own floor, so three levels can meet in one run and 12 + 12 + 6 checks is
 * 30 grading calls for a single deliverable. When the budget binds, the GENERIC checks are the ones
 * dropped — the specific standards are the reason the agent-scoped rubric exists at all.
 */
const MAX_TOTAL_CHECKS = 16;
...
function mergeRubric(handbookChecks, floorRubric, meta = {}) {
  ...
  const specific = handbookChecks.slice(0, MAX_TOTAL_CHECKS);
  const seen = new Set(specific.map((c) => c.id));
  const kept = floor
    .filter((c) => !seen.has(c.id))
    .map((c) => ({ ...c, source: c.source || 'rubric' }))
    .slice(0, Math.max(0, MAX_TOTAL_CHECKS - specific.length));
  ...
  return {
    ...
    checks: [...specific, ...kept],
    handbookCheckCount: specific.length,
