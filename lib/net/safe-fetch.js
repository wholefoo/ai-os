...
// One pinned request. Connects to the pre-validated IP (no second name resolution), keeping the
// real hostname for Host + TLS SNI. Resolves { redirect } or { status, statusText, body }, or rejects.
//
// `method`/`body` exist for callers that must POST (webhook test-fire). `textOnly` and `okOnly`
// keep safeFetch's original strictness — a non-200 or a binary content-type rejects — while
// safeRequest turns both off, because reporting "that endpoint answered 500" IS its result rather
// than a failure. The SSRF guarantees do not vary with any of these flags; only what counts as an
// acceptable ANSWER does.
function requestPinned(targetUrl, pin, { timeoutMs, maxBytes, headers, method = 'GET', body = null, textOnly = true, okOnly = true, followRedirects = true }) {
  return new Promise((resolve, reject) => {
    ...
    const req = mod.request(u, {
      ...
      if (status >= 300 && status < 400 && res.headers.location && followRedirects) {
        res.resume(); finish({ redirect: new URL(res.headers.location, u).href }); return;
      }
      if (okOnly && status !== 200) { res.resume(); fail(new Error(`fetch returned ${status}`)); return; }
      const ct = String(res.headers['content-type'] || '');
      if (textOnly && ct && !/text|html|css|xml|json|javascript|octet-stream/i.test(ct)) { res.resume(); fail(new Error('unsupported content-type')); return; }
      let stream = res;
      ...
      const settleBody = () => finish({ status, statusText, body: Buffer.concat(chunks).toString('utf-8') });
      stream.on('data', (c) => {
        received += c.length;
      ...
      stream.on('end', settleBody);
      stream.on('error', settleBody);
    });
...
/**
 * SSRF-safe request that REPORTS the response instead of demanding a good one.
 *
 * safeFetch answers "give me the text at this URL" and throws on anything else. Some callers are
 * asking a different question — "what happens when I call this endpoint?" — where a 500 or an HTML
 * error page is the answer, not a failure. The plugin test-fire is the case that forced this: it
 * POSTs a payload and reports the status back to the operator, so safeFetch's throw-on-non-200 and
 * text-only gate would turn a useful diagnostic into "request failed".
 *
 * The SSRF guarantees are IDENTICAL and non-optional — same resolve-once, same private-IP rejection
 * on every answer, same pin to the validated address. Only the definition of an acceptable RESPONSE
 * is relaxed. That split is the point: a caller can loosen what it accepts without ever being able
 * to loosen where it may connect.
 *
 * Redirects are NOT followed for non-GET methods. Replaying a body to a second host after a 302 is
 * how a "safe" client turns into a confused deputy, and a redirected POST is not something the
 * caller asked for. The hop is reported as its 3xx status instead.
 *
 * @returns {Promise<{status:number, statusText:string, body:string}>} throws only on SSRF refusal,
 *          an unusable URL, or a transport failure.
 */
async function safeRequest(rawUrl, { method = 'GET', body = null, headers = {}, maxBytes = 1_000_000, timeoutMs = 10000, maxRedirects = 4, userAgent } = {}) {
  const dl = Date.now() + timeoutMs;
  const verb = String(method || 'GET').toUpperCase();
  const hdrs = { 'User-Agent': userAgent || DEFAULT_UA, Accept: '*/*', 'Accept-Encoding': 'identity', ...headers };
  if (body != null && !Object.keys(hdrs).some((h) => h.toLowerCase() === 'content-length')) {
    hdrs['Content-Length'] = Buffer.byteLength(body);
  }
  const followRedirects = verb === 'GET' || verb === 'HEAD';
  ...
  for (let hop = 0; hop <= maxRedirects; hop++) {
    ...
    const pin = await resolvePinned(u.hostname);
    const r = await requestPinned(u.href, pin, {
      timeoutMs: Math.min(remaining, 12000), maxBytes, headers: hdrs,
      method: verb, body, textOnly: false, okOnly: false, followRedirects,
    });
    if (r.redirect) { url = r.redirect; continue; }
...
module.exports = { safeFetch, safeRequest };
