...
const MAX_PDF_PAGES = 500;                   // same idea for PDFs: one enormous document cannot fill the cap alone
 ...
 */
const SUPPORTED = ['txt', 'md', 'csv', 'docx', 'xlsx', 'pdf'];

const REFUSALS = {
  // 'pdf' used to live here. It is now SUPPORTED via unpdf — see fromPdf below.
...
/**
 * .pdf via unpdf (a pure-JS repackaging of Mozilla's pdf.js with the rendering/canvas paths removed).
 *
 * WHY unpdf AND NOT pdf-parse, which is the usual first choice: pdf-parse@2 depends on
 * `@napi-rs/canvas`, a Skia-backed NATIVE binding with per-platform binaries. This repo deliberately
 * avoids native dependencies, and a native dep would make installs platform-specific on the VPS.
 * unpdf is MIT, has zero runtime dependencies, and is ~2 MB against pdfjs-dist's ~34 MB — the
 * difference being precisely the rendering machinery we never use, since we want text and never
 * pixels.
 *
 * Required lazily, matching adm-zip and exceljs above. That is not stylistic: a top-level require
 * would turn a missing or incompatible install into a SERVER BOOT FAILURE, where this way it is a
 * clean per-upload error and every other format keeps working. unpdf needs Node >= 22.
 *
 * Pages are labelled. A sheet name earns its label because it carries meaning; a page number earns
 * its own because this department exists to be quoted from, and "which page" is the first thing
 * anyone asks of a citation.
 */
async function fromPdf(buffer) {
  const { extractText } = require('unpdf');
  // A fresh Uint8Array rather than the Buffer itself: pdf.js may transfer/detach the backing buffer,
  // and the caller still owns theirs (extract() hashes the same bytes for provenance).
  const { totalPages, text } = await extractText(new Uint8Array(buffer), { mergePages: false });
  const pages = Array.isArray(text) ? text : [String(text == null ? '' : text)];

  const out = [];
  const kept = Math.min(pages.length, MAX_PDF_PAGES);
  for (let i = 0; i < kept; i += 1) {
    const t = String(pages[i] == null ? '' : pages[i]).trim();
    if (t) out.push(`## Page ${i + 1}\n${t}`);
  }
  // Truncation is ANNOUNCED IN THE OUTPUT, the same way a truncated sheet is. Silently dropping the
  // back half of a contract is the kind of helpfulness that gets someone in trouble.
  if (pages.length > MAX_PDF_PAGES) {
    out.push(`… truncated at ${MAX_PDF_PAGES} of ${totalPages} pages`);
  }
  return tidy(out.join('\n\n'));
}
...
async function extract({ filename, buffer } = {}) {
  ...
  try {
    ...
    else if (format === 'pdf') text = await fromPdf(buffer);
...
module.exports = {
  ...
  MAX_UPLOAD_BYTES,
  MAX_TEXT_CHARS,
  MAX_PDF_PAGES,   // exported so the test asserts against the REAL ceiling, not a copy of the number
  extensionOf,
  isSupported,
  refusalFor,
  extract,
