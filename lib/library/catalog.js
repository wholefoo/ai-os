...
//    the instance.
//  - NO ACCESS DECISIONS. `search()` and `filter()` are plain predicates over whatever record array
...
//    read it) is still returned by search() — proving the boundary is real, not just a comment.
//
//  normalize() is the schema gate: every LibraryRecord that ever enters the catalog passes through
...
//  the schema names it an always-false INVARIANT (§4), and a `true` value reaching normalize() means
...
//
//  Pure module: shapes, normalize, dedupe, search/filter predicates. No state, no I/O, no requires
//  beyond node's own `crypto` (used only to backstop a missing `id` — see idOrGenerate below).
// ============================================================
...
/** Does one record match a search query, over title/tags/source/format only. See search()'s header
 ...
 * readers.readableBy() itself; search() answers only "what matches this text", the same way a
 ...
 * omitted criterion imposes no constraint. Like search(), this does not filter by `readers`.
 *
 * @param {object[]} records
