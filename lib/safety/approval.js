...
const ACTION_RISK = {
  ...
  // The LEGACY enterprise proposal-apply path (server.js applyProposal — npm update/audit-fix,
  // config writes, content refresh, new skills). It is NOT dispatched through gateAction: every
  // call site already sits behind an explicit human approval of the proposal itself (dashboard
  // admin, the /apply endpoint, or Telegram /approve), and routing it through gateAction would
  // demand a SECOND approval for the same decision — the exact fatigue antipattern the 2026-08-28
  // agent-overhead audit catalogued. Registered here so the risk registry NAMES the action, the
  // handbook validator can check it, and apply results can carry a canonical actionId.
  'self-improve.apply-proposal': 'critical',
