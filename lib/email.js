...
// send({cfg, to, subject, text, unsubscribeUrl}) -> { ok, id?, error? }
// `text` is the whole authored body; HTML is derived. Never throws — callers branch on ok.
async function send({ cfg, to, subject, text, unsubscribeUrl = null }) {
  try {
    ...
    const html = textToHtml(text, { footerHtml });
    ...
    if (provider === 'resend') {
      const r = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({ from, to: [to], subject, html, text: plain, headers }),
      });
    ...
    const info = await transporter.sendMail({ from, to, subject, html, text: plain, headers });
