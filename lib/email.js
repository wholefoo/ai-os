...
// send({cfg, to, subject, text, unsubscribeUrl, attachments}) -> { ok, id?, error? }
// `text` is the whole authored body; HTML is derived. attachments: [{ filename, content }]
// where content is a utf-8 string or Buffer. Never throws — callers branch on ok.
async function send({ cfg, to, subject, text, unsubscribeUrl = null, attachments = [] }) {
  try {
