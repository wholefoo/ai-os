...
function isConfigured(cfg = {}) {
  ...
  return !!(cfg.from_email && (provider === 'resend' ? cfg.resend_api_key : provider === 'smtp' && cfg.smtp_host));
}
...
async function send({ cfg, to, subject, text, html: customHtml = null, unsubscribeUrl = null, attachments = [], transactional = false, idempotencyKey }) {
  try {
    const provider = pickProvider(cfg);
    if (!isConfigured(cfg)) return { ok: false, error: 'email not configured — set a Resend key or SMTP host + a From address in Settings → Email' };
    const from = cfg.from_name ? `${cfg.from_name} <${cfg.from_email}>` : cfg.from_email;
    const footerHtml = transactional ? '' : complianceFooter({ fromName: cfg.from_name, footerAddress: cfg.footer_address, unsubscribeUrl });
    ...
    if (provider === 'resend') {
      ...
      const r = await fetch('https://api.resend.com/emails', {
        ...
        headers: { Authorization: `Bearer ${cfg.resend_api_key}`, 'Content-Type': 'application/json', ...(idempotencyKey ? { 'Idempotency-Key': idempotencyKey } : {}) },
