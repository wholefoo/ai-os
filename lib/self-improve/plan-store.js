...
const DENIED_PATH_PREFIXES = [
  '.env', '.git/', 'node_modules/', '.magent/state/', '.magent/vault/raw/', '.magent/artifacts/', '.magent/provenance/',
...
function isReadPathAllowed(relPath) {
  const norm = normalizeRelPath(relPath);
  // Repository readers must never expose private runtime data. Internal runtime writers
  // retain their separate containment policy (e.g. the reasoning trace store).
  if (norm === '.magent' || norm?.startsWith('.magent/')) return false;
