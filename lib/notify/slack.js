 ...
 *
 * Named `webhookReady` rather than `isConfigured` because lib/email.js already exports an
 * `isConfigured`, and two files exporting one name is what the dead-code gate flags as a
 * duplicate export. That warning is not pedantry here: the last time someone renamed to clear
 * one, they also rewrote a call site pointing at a DIFFERENT module and took the clone
 * interview down in production. This rename was done by mapping every call site first — there
 * was exactly one, plus its tests.
 */
function webhookReady(raw) {
...
module.exports = { resolveWebhook, webhookReady, configWarning, notificationPayload, approvalPayload };
