...
const TEMPLATES = [
  ...
  },
  {
    ...
    requires: ['Set {{AI_OS_TOKEN}} on the HTTP node to an AI OS service key with scope agent (POST /api/admin/service-keys) — not the master API_TOKEN', 'Adjust the schedule + the agent/task body'],
  ...
  },
  {
    ...
    requires: ['A Gmail credential in n8n', 'Set {{AI_OS_TOKEN}} on the HTTP node to an AI OS service key with scope agent (POST /api/admin/service-keys) — not the master API_TOKEN'],
