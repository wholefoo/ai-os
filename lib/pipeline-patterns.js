...
async function runPattern(stage, ctx, deps) {
  ...
  switch (stage.pattern) {
    ...
    case 'skeptic': {
      ...
      const lines = (r.verdicts || []).map((v, i) => `${i + 1}. ${v.answered ? (v.refuted ? 'REFUTED' : 'SOUND') : 'NO VERDICT'} — ${v.reason}`).join('\n');
      const output = `Adversarial panel: ${r.refuteCount}/${r.answered} refuted (n=${r.n}, quorum ${r.quorum}).\n${lines}`;
      // A panel below quorum is NOT a pass. Same rule as the repo's own adversarial-verification.md:
      // a missing verdict is a block, never a silent ship — and "missing" includes a reply with no
      // verdict in it, not only an errored call.
      if (r.inconclusive) return { ok: false, output: output + `\nInconclusive: ${r.answered} of ${r.n} skeptics gave a verdict (quorum ${r.quorum}) — treated as a block.`, meta: r, verdict: 'inconclusive' };
      if (r.refuted) {
