...
//
// WHY THIS AND `runSequential` BELOW HAVE NO CALLERS, and are allowlisted in `.fallowrc.json`
// under `ignoreExports`. G2 wired five of the orchestration patterns to pipeline `pattern:` stages;
// these two are the ones that genuinely cannot be:
//   - `loopUntilDone` takes FUNCTION arguments (stepFn/isDone), which YAML cannot express.
//   - `runSequential` is what the pipeline runner already does, so wiring it would be circular.
// They are kept as a library surface, not dead code.
//
