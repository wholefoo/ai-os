...
//
// THREE-VALUED, like lib/reasoning's verifier: refuted / sound / INCONCLUSIVE. The first version was
// fail-open in three ways, all of which read as "not refuted" downstream (SOC 2 gap item 19):
//   1. an errored skeptic was simply dropped, so with n=3 two errors left ONE voice deciding;
//   2. all-errored returned neither refuted nor sound, which callers checking only `.refuted`
//      treated as a pass;
//   3. a reply with no verdict word in it — empty, or cut off at max_tokens before the first line
//      was written (adaptive thinking spends from the same budget) — counted as a SOUND vote,
//      because "not containing REFUTED" was the test.
// Now: a skeptic has ANSWERED only if its reply carries a parseable verdict; the panel needs a
// QUORUM of answers (default: strict majority of n) or it is inconclusive; and inconclusive is
// its own flag that no caller may mistake for sound. The refute prompt tells the model to put
// the verdict on the first line, so the first line is what is parsed.
async function adversarialVerify(subject, deps, opts = {}) {
  const d = dep(deps);
  const n = Math.max(1, opts.n || 3);
  const quorum = Math.min(n, Math.max(1, opts.quorum || Math.floor(n / 2) + 1));
  ...
  // No maxTokens cap here on purpose: the reply is two lines, but a cap of a few hundred tokens is
  // shared with the model's own thinking and produced empty replies. executeAgent's default floor applies.
  const verdicts = await Promise.all(Array.from({ length: n }, () =>
    call(d, verifier, prompt, opts.agentOpts || {}).then((r) => {
      const t = text(r);
      const firstLine = t.split('\n').find((l) => l.trim()) || '';
      const m = firstLine.match(/\b(REFUTED|SOUND)\b/i) || t.match(/\b(REFUTED|SOUND)\b/i);
      const verdict = m ? m[1].toUpperCase() : null;
      const truncated = !!(r && (r.truncated === true || r.stopReason === 'max_tokens'));
      const answered = !!r.ok && !!verdict; // a truncated reply that still carried its first-line verdict counts; one cut off before it does not
      const reason = firstLine || r.error || (truncated ? 'reply truncated before a verdict' : 'no verdict in reply');
      return { ok: !!r.ok, answered, refuted: answered && verdict === 'REFUTED', verdict: answered ? verdict : null, truncated, reason };
    })));
  const answered = verdicts.filter((v) => v.answered).length;
  const errored = verdicts.filter((v) => !v.ok).length;
  const unparsed = verdicts.filter((v) => v.ok && !v.answered).length;
  ...
  const inconclusive = answered < quorum;
  return {
    verdicts, refuteCount, answered, errored, unparsed, n, quorum, inconclusive,
    refuted: !inconclusive && refuteCount >= threshold,
    sound: !inconclusive && refuteCount < threshold,
  };
}
