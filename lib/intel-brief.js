...
// ---------- markdown-ish → docx ----------
// The comms-director writes prose with #/## headings and - bullets; markdownToParagraphs (shared
// with lib/pipeline-reports.js via lib/docx-markdown.js) renders those faithfully and treats
// everything else as body paragraphs — a weird model output degrades to plain paragraphs, never a
// crash.
function statementToDoc({ title, dateLabel, statement }) {
  const children = [
    ...
    ...markdownToParagraphs(statement),
  ];
  return new Document({
...
async function saveBriefDocx({ dir, kind, statement, extraMeta = {} }) {
  ...
  return writeDocxPair({
    dir, file, doc: statementToDoc({ title: `AI OS ${KINDS[kind]}`, dateLabel, statement }),
    meta: {
...
function deleteBrief(dir, file) {
  return deleteDocxPair(dir, file, FILE_RE);
}

function listBriefs(dir) {
  return listDocxPairs(dir, FILE_RE, (f, meta) => {
    const kind = meta.kind || ((f.match(FILE_RE) || [])[1]) || 'intel-brief';
    return {
      kind, kindLabel: KINDS[kind] || kind,
      date: meta.date || ((f.match(/(\d{4}-\d{2}-\d{2})/) || [])[1] || ''),
      consultantsReported: meta.consultantsReported ?? null,
    };
  });
}
