...
const FILE_RE = /^intel-brief-\d{4}-\d{2}-\d{2}(-\d+)?\.docx$/; // download route allowlist
...
async function runIntelBrief(deps, { dir, useMcpTools = true } = {}) {
  ...
  // 5. Render + persist (never overwrite an earlier same-day brief — suffix instead).
  fs.mkdirSync(dir, { recursive: true });
  let file = `intel-brief-${date}.docx`;
  for (let n = 2; fs.existsSync(path.join(dir, file)); n++) file = `intel-brief-${date}-${n}.docx`;
  const doc = statementToDoc({ title: 'AI OS Daily Intelligence Statement', dateLabel, statement: comms.content });
  fs.writeFileSync(path.join(dir, file), await Packer.toBuffer(doc));

  const meta = {
    file, date, createdAt: new Date().toISOString(),
    consultantsReported: reported, consultantsTotal: CONSULTANTS.length,
    summary: String(comms.content).replace(/^#.*$/gm, '').trim().slice(0, 500),
  };
  fs.writeFileSync(path.join(dir, file.replace(/\.docx$/, '.json')), JSON.stringify(meta, null, 2));
  log(`[intel-brief] wrote ${file}`);
  return meta;
}
  ...
  try {
    return fs.readdirSync(dir)
      ...
      .map((f) => {
        ...
        return { file: f, size: stat.size, createdAt: meta.createdAt || stat.mtime.toISOString(), date: meta.date || f.slice(11, 21), consultantsReported: meta.consultantsReported ?? null, summary: meta.summary || '' };
      })
      .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  } catch { return []; }
}

module.exports = { runIntelBrief, listBriefs, FILE_RE, CONSULTANTS };
