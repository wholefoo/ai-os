...
function computeOversight(approvals, counters = {}, { now = Date.now(), windowDays = 30 } = {}) {
  ...
  // Reject-reason aggregation (audit P4): what keeps getting refused, in the operator's own words.
  // Normalised case/whitespace so "Too risky" and "too risky " are one pattern; capped at the top 5
  // because this is a signal, not a log. `withReason`/`withoutReason` are both reported — a low
  // with-reason share means the prompt is being skipped, which is itself worth seeing.
  const rejectedInWindow = inWindow.filter((a) => a.rejectedAt);
  const reasonCounts = {};
  for (const a of rejectedInWindow) {
    const raw = String(a.rejectReason || '').trim().toLowerCase().replace(/\s+/g, ' ');
    if (!raw) continue;
  ...
  return {
    ...
    decided: {
      ...
      rejectReasons: {
        withReason: Object.values(reasonCounts).reduce((a, b) => a + b, 0),
        withoutReason: rejectedInWindow.length - Object.values(reasonCounts).reduce((a, b) => a + b, 0),
        top: topRejectReasons,
      },
    },
    autoVsGated: {
