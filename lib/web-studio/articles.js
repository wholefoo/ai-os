...
function isoOrNull(v) {
  ...
  // A bare YYYY-MM-DD (what an <input type=date> sends) is anchored at MIDDAY UTC. new Date() reads
  // it as midnight UTC, which displays as the PREVIOUS day everywhere in the Americas — so every
  // save through the editor was quietly moving articles a day earlier. Adoption and ingest already
  // did this; the editor path went through here and never had it.
  const d = new Date(/^\d{10}$/.test(s) ? Number(s) * 1000 : (/^\d{4}-\d{2}-\d{2}$/.test(s) ? s + 'T12:00:00Z' : s));
...
function normalizeArticle(input, opts = {}) {
  ...
  if (kind === 'video') {
    if (has('youtubeId') || has('youtubeUrl')) {
      const raw = has('youtubeId') ? a.youtubeId : a.youtubeUrl;
      // null / '' CLEARS it, symmetric with videoUrl and duration. Without this, carry-over meant a
      // user switching a video from YouTube to an MP4 in the editor could never remove the old id.
      if (raw === null || raw === '') videoYoutubeId = null;
      else {
  ...
  return {
    ...
    // `now` applies only at CREATION. An existing entry keeps whatever it had — including null. The
    // previous `existing.publishedAt || now` chain meant that editing an UNDATED entry (every adopted
    // article whose source had no date) stamped it with today on the first save, silently undoing
    // the fabricated-date fix through the editor.
    publishedAt: isoOrNull(a.publishedAt)
      || (existing ? (existing.publishedAt || null) : (opts.undatedOk ? null : now)),
    updatedAt: now,
...
/**
 * The newsletter block appended to article and video pages, or null. OPT-IN: only when the plan sets
 * newsletter.enabled, so no existing site grows a signup form on its next rebuild.
 *
 * The default copy claims only what is TRUE. Web Studio does not email subscribers when a post is
 * published, so the hub's default ("One email when something new is published") would be a false
 * promise in native mode. The unsubscribe sentence is added only for native mode, where lib/email.js
 * provably appends a signed one-click unsubscribe link to every message; an external provider's
 * behaviour is its own, so the operator writes that copy.
 */
function newsletterSection(plan) {
  const n = plan && plan.newsletter;
  if (!n || n.enabled !== true) return null;
  const external = /^https:\/\//i.test(String(n.action || ''));
  const site = plan.siteName || 'us';
  return {
    type: 'newsletter',
    heading: n.heading || 'Stay in touch',
    blurb: n.blurb || ('Occasional emails from ' + site + '.' + (external ? '' : ' One-click unsubscribe in every email.')),
    button: n.button || 'Subscribe',
    action: external ? n.action : '',
  };
}
...
function expandArticlePages(plan) {
  ...
  const signup = newsletterSection(plan);
  const withSignup = (sections) => (signup ? [...sections, { ...signup }] : sections);
