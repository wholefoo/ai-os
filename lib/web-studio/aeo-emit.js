...
// Page URLs carry a TRAILING SLASH, because that is what the site actually serves. Astro is pinned
// to `build: { format: 'directory' }` (scaffold.js), so /about is emitted as dist/about/index.html
// and nginx 301s /about -> /about/. Emitting the slashless form meant the canonical tag pointed at a
// URL that REDIRECTED to the page carrying it — so it was not self-referencing — and every sitemap
// entry was a redirect hop, which search engines report as "Page with redirect" and discount. That
// is the opposite of what an AEO product should ship. Found on two LIVE sites after publishing, by
// following redirects; a status check that did not follow them showed 301 and looked survivable.
// This is the single chokepoint: canonical, page JSON-LD url, llms.txt links, the OKF resource and
// the sitemap all route through here. tools/test-canonical-slash.js pins it.
function pageUrl(domain, p) {
  ...
  // A final segment containing a dot is served as a FILE, not a directory — never slash those.
  const last = clean.slice(clean.lastIndexOf('/') + 1);
  return u + clean + (last.includes('.') ? '' : '/');
}
...
// schema.org FAQPage from the page's faq section(s) — the same q/a pairs the visible
// <details> accordion renders, so the structured data can never drift from the page content.
// Null when the page has no faq items (never emit an empty FAQPage).
function faqLdObject(page) {
  const qa = [];
  for (const s of ((page && page.sections) || [])) {
    if (!s || s.type !== 'faq' || !Array.isArray(s.items)) continue;
    for (const it of s.items) {
      if (it && it.q && it.a) qa.push({ '@type': 'Question', name: String(it.q), acceptedAnswer: { '@type': 'Answer', text: String(it.a) } });
    }
  }
  ...
  for (const p of (plan.pages || [])) {
    ...
    for (const s of (Array.isArray(p.sections) ? p.sections : [])) {
      ...
      if (Array.isArray(s.items)) {
        for (const it of s.items) {
          if (it.q && it.a) sectionLines.push(`- **${it.q}** ${it.a}`);                                             // faq
          else if (it.quote) sectionLines.push(`- "${it.quote}" — ${[it.name, it.role].filter(Boolean).join(', ')}`); // testimonials
          else if (it.price) sectionLines.push(`- **${it.name || ''}**: ${it.price}${it.period ? ' ' + it.period : ''}${Array.isArray(it.features) && it.features.length ? ' — ' + it.features.join(', ') : ''}`); // pricing
          else if (it.value && it.label) sectionLines.push(`- **${it.value}** ${it.label}`);                          // stats
          else if (it.bio || it.role) sectionLines.push(`- **${it.name || it.title || ''}**${it.role ? ` (${it.role})` : ''}${it.bio ? ': ' + it.bio : ''}`); // team
          else sectionLines.push(`- **${it.title || it.name || ''}**${it.body ? ': ' + it.body : ''}`);               // features/steps
        }
        sectionLines.push('');
      }
...
module.exports = { siteLdObjects, pageLdObject, canonicalUrl, llmsTxt, robotsTxt, sitemapXml, xesc, provenanceSidecar, actionLdObjects, faqLdObject, okfBundle };
