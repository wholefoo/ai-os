...
function command(args, timeout) {
  return new Promise((resolve, reject) => {
    execFile('docker', args, { timeout, windowsHide: true, encoding: 'buffer', maxBuffer: LIMIT }, (error, stdout, stderr) => {
...
async function isolatedBuild(workspace, timeoutMs) {
  ...
  if (!image) throw new Error('Isolated build worker is not configured. Set AIOS_BUILD_IMAGE to the locally installed Web Studio build image.');
  ...
  try {
    ...
    const result = await command(['run', '--rm', '--pull=never', '--log-driver=none', '--name', name,
      ...
      'set -eu; cp -R /input /work/site; cp -R /opt/build/node_modules /work/site/node_modules; cd /work/site; ./node_modules/.bin/astro build >&2; tar --format=ustar -C dist -cf - .'], timeoutMs);
    ...
    await command(['rm', '-f', name], 10000).catch(() => {});
