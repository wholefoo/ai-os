...
function createVhost(domain, opts = {}) {
  ...
  // The script REFUSES to flip an existing site between http-only and TLS unless told to. That
  // guard exists because doing it by accident is an outage (see the comment in site-vhost.sh), so
  // only a caller that genuinely intends the promotion may opt out of it — never by default.
  if (opts.allowSchemeChange) args.push('--allow-scheme-change');
  if (opts.preserveExisting) args.push('--preserve-existing');
...
async function attachDomainWithTls(domain) {
  const d = normalizeDomain(domain);
  await createVhost(d, { tls: false, preserveExisting: true });
  await issueCert(d);                   // throws if challenge fails; site stays HTTP
  // http -> tls IS a scheme change, and the only sanctioned one: we just created the HTTP vhost two
  // steps ago specifically so ACME could complete. Opting out of the guard here is deliberate and
  // must stay confined to this flow — it is the single place that knows the promotion is intended.
  await createVhost(d, { tls: true, allowSchemeChange: true });  // promote to HTTPS
  return d;
}
