...
function renderSection(section, opts = {}) {
  ...
  try { return SECTIONS[type]({ ...(section || {}), _extraClass: extraClass, _motion: !!opts.motion, _leadEndpoint: opts.leadEndpoint || '' }); } catch { return SECTIONS.prose({ body: '' }); }
}
...
function renderPage(page, meta, plan = {}) {
  ...
  const sections = (Array.isArray(page.sections) ? page.sections : []).map((s) => renderSection(s, { motion, leadEndpoint: plan.leadEndpoint })).join('\n');
...
Allowed section types: hero, features, prose, cta, contact, testimonials, pricing, faq, stats, team, steps. Use real, on-brand copy (NO lorem). Keep it to 1-3 pages for the first build.
...
async function createSiteFromBrief(opts, deps) {
  const { workspaceDir, brief, siteId, domain, siteName, design, scraped, research, affiliateUrl, checkoutUrl, features, platformBaseUrl, modelOverride, templatePlan } = opts;
