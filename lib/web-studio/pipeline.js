...
const { expandArticlePages } = require('./articles');
...
${rows}
  ...
  // The article index. `features` was not reusable here: its cards carry no link, and a listing
  // whose entries are not clickable is not a listing.
  articleList(s) {
    const items = Array.isArray(s.items) ? s.items : [];
    const cards = items.map((it) => `      <li class="border-b border-ink/10 pb-6">
${it.image ? `        <a href="${safeHref(it.href)}" tabindex="-1" aria-hidden="true"><img src="${safeHref(it.image)}" alt="" loading="lazy" class="mb-4 w-full rounded-lg object-cover" /></a>\n` : ''}${it.category ? `        <p class="text-xs font-medium uppercase tracking-wide text-brand">${esc(it.category)}</p>\n` : ''}        <h3 class="mt-1 text-xl font-semibold text-ink"><a href="${safeHref(it.href)}" class="hover:underline">${esc(it.title)}</a></h3>
${it.excerpt ? `        <p class="mt-2 text-ink/70">${esc(it.excerpt)}</p>\n` : ''}${it.meta ? `        <p class="mt-2 text-sm text-ink/60">${esc(it.meta)}</p>\n` : ''}      </li>`).join('\n');
  ...
  <div class="mx-auto max-w-2xl">
    ...
    ${items.length ? `<ul class="mt-10 grid gap-8">\n${cards}\n    </ul>` : '<p class="mt-6 text-ink/60">No articles yet.</p>'}
  </div>
</section>`;
  },
  article(s) {
...
// Does any page in the plan use the article section? Drives the conditional prose stylesheet.
function planHasArticle(plan) {
  return (Array.isArray(plan && plan.pages) ? plan.pages : [])
    .some((p) => (Array.isArray(p && p.sections) ? p.sections : []).some((s) => s && s.type === 'article'));
}
