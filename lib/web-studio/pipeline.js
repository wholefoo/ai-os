...
function planPrompt(brief, { domain, siteName, design, scraped } = {}) {
...
function contentPrompt(brief, plan, { domain, scraped } = {}) {
...
Keep titles unique per page. No lorem. Base it on the brief and the existing plan.${domain ? `\nUse the domain ${domain} for any URL or email; never invent a different domain.` : ''}${scrapedContentNote(scraped)}
...
async function createSiteFromBrief(opts, deps) {
  const { workspaceDir, brief, siteId, domain, siteName, design, scraped, features, platformBaseUrl } = opts;
  ...
  // Scraped content is untrusted external text — fenced via executeAgent's `untrusted` option (nonce
  // fence + system guard), never string-concatenated into the prompt. Shared by both agent calls below.
  const scrapedUntrusted = scrapedUntrustedBlocks(scraped);
  ...
  emit('planning'); log(`[web-studio] planning ${siteId}${design ? ' (design-cloned)' : ''}${scraped ? ' (redesign)' : ''}`);
  const planResp = await executeAgent('web-studio-lead', planPrompt(brief, { domain, siteName, design, scraped }), { maxTokens: 16000, untrusted: scrapedUntrusted });
  ...
  try {
    const cResp = await executeAgent('content-writer', contentPrompt(brief, plan, { domain, scraped }), { maxTokens: 8000, untrusted: scrapedUntrusted });
