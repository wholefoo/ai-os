...
<html lang="en">
  <head>
    ...
    {siteLd.map((d) => <script type="application/ld+json" set:html={JSON.stringify(d)} />)}
    <slot name="head" />
  </head>
...
const pageLd = ${JSON.stringify(pageLd)};
---
...
function planPrompt(brief, { domain, siteName, design } = {}) {
...
${siteName ? `Use "${siteName}" as the brand/company name (the "siteName" field and throughout the copy) — do NOT invent a different brand name.\n` : ''}${domain ? `The site's real domain is ${domain}. Use this EXACT domain anywhere a URL, link, contact email (e.g. hello@${domain}), or the footer needs one — NEVER invent or use a different domain.` : 'No domain is set — do NOT invent one; use relative links ("/", "/#contact") and avoid made-up email addresses.'}${designRefBlock(design)}
...
function contentPrompt(brief, plan, { domain } = {}) {
...
async function createSiteFromBrief(opts, deps) {
  const { workspaceDir, brief, siteId, domain, siteName, design } = opts;
  ...
  emit('planning'); log(`[web-studio] planning ${siteId}${design ? ' (design-cloned)' : ''}`);
  const planResp = await executeAgent('web-studio-lead', planPrompt(brief, { domain, siteName, design }), { maxTokens: 16000 });
  ...
  // Design clone: force the extracted palette/fonts onto the plan so the cloned look is
  // applied deterministically even if the agent drifted (tokensCss sanitizes the values).
  ...
  try {
    const cResp = await executeAgent('content-writer', contentPrompt(brief, plan, { domain }), { maxTokens: 8000 });
...
module.exports = { createSiteFromBrief, renderPlanToWorkspace, writeProvenanceSidecar, extractJson, renderPage, renderBase, renderSection };
