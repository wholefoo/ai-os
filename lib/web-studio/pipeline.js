...
function planPrompt(brief, { domain, siteName, design, scraped, research } = {}) {
...
async function createSiteFromBrief(opts, deps) {
  const { workspaceDir, brief, siteId, domain, siteName, design, scraped, research, affiliateUrl, features, platformBaseUrl } = opts;
  ...
  const planResp = await executeAgent('web-studio-lead', planPrompt(brief, { domain, siteName, design, scraped, research }), { maxTokens: 16000, untrusted: scrapedUntrusted });
  ...
  try {
    const cResp = await executeAgent('content-writer', contentPrompt(brief, plan, { domain, scraped, research }), { maxTokens: 8000, untrusted: scrapedUntrusted });
