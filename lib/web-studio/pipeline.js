...
async function createSiteFromBrief(opts, deps) {
  ...
  // 1. Plan (web-studio-lead)
  emit('planning'); log(`[web-studio] planning ${siteId}`);
  const planResp = await executeAgent('web-studio-lead', planPrompt(brief));
  const plan = extractJson(planResp && planResp.content);
  if (!plan || !Array.isArray(plan.pages) || plan.pages.length === 0) {
    emit('failed', { error: 'plan' });
    return { ok: false, status: 'plan_failed', error: 'web-studio-lead did not return a usable plan' };
  }
  ...
  let meta = {};
  try {
    const cResp = await executeAgent('content-writer', contentPrompt(brief, plan));
