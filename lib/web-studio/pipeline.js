...
const { expandArticlePages, rssXml, planHasFeed } = require('./articles');
...
${rows}
  ...
  // The ONLY section that emits author-supplied markup rather than escaped text. Everything else
  // here runs content through esc(), which is why an imported blog could never be edited in the
  // no-code editor: its bodies ARE html (headings, links, footnote anchors), and escaping renders
  // the tags as visible literal text.
  //
  // Two hard requirements, both load-bearing:
  //   1. SANITISE. The body reaches a published customer domain, where a script would run with that
  //      origin's privileges. sanitizeHtml is an allowlist and is idempotent, so re-sanitising here
  //      — even though the API already did on write — costs nothing and closes the gap for any
  //      body that reached the plan by another route (import, adoption, a restored backup).
  //   2. NEUTRALISE BRACES. This string is embedded in a .astro template, where `{expr}` is a JS
  //      expression — a literal `{` in prose would be parsed as code and break the build (the same
  //      trap aeo-emit.js documents for raw JSON-LD). `&#123;`/`&#125;` render as the literal
  //      characters, so the reader sees no difference.
  // The article index — and, since the hub integration, every listing: videos, a tag's entries,
  // Start here. `features` was not reusable here: its cards carry no link, and a listing whose
  // entries are not clickable is not a listing.
  articleList(s) {
    const items = Array.isArray(s.items) ? s.items : [];
    const cards = items.map((it) => {
      const badge = it.kind === 'video'
        ? `<span class="ws-badge">Video${it.duration ? ' · ' + esc(it.duration) : ''}</span>` : '';
      return `      <li class="border-b border-ink/10 pb-6">
${it.image ? `        <a href="${safeHref(it.href)}" tabindex="-1" aria-hidden="true" class="ws-thumb${it.kind === 'video' ? ' ws-thumb-video' : ''}"><img src="${safeHref(it.image)}" alt="" loading="lazy" class="mb-4 w-full rounded-lg object-cover" /></a>\n` : ''}${it.category || badge ? `        <p class="text-xs font-medium uppercase tracking-wide text-brand">${badge}${it.category ? esc(it.category) : ''}</p>\n` : ''}        <h3 class="mt-1 text-xl font-semibold text-ink"><a href="${safeHref(it.href)}" class="hover:underline">${esc(it.title)}</a></h3>
${it.excerpt ? `        <p class="mt-2 text-ink/70">${esc(it.excerpt)}</p>\n` : ''}${it.meta ? `        <p class="mt-2 text-sm text-ink/60">${esc(it.meta)}</p>\n` : ''}${tagChips(it.tags)}      </li>`;
    }).join('\n');
    return `<section class="px-6 py-16${s._extraClass || ''}">
  <div class="mx-auto max-w-2xl">
    ${s.eyebrow ? `<p class="text-sm font-medium uppercase tracking-wide text-brand">${esc(s.eyebrow)}</p>` : ''}
    ${s.heading ? `<h1 class="text-3xl font-display font-bold text-ink">${esc(s.heading)}</h1>` : ''}
    ${s.intro ? `<p class="mt-3 text-lg text-ink/70">${esc(s.intro)}</p>` : ''}
    ${items.length ? `<ul class="mt-10 grid gap-8">\n${cards}\n    </ul>` : `<p class="mt-6 text-ink/60">${esc(s.emptyText || 'No articles yet.')}</p>`}
  </div>
</section>`;
  },
  // Newsletter signup (the hub integration). Two destinations:
  //   external — a provider's form endpoint (Buttondown and similar accept a plain POST with an
  //              `email` field). Must be https. With a provider that does RSS-to-email, the site's
  //              /rss.xml gives subscribers an email per new post without Web Studio sending bulk mail.
  //   native   — the site's own hardened lead route, marked kind=newsletter so the person is recorded
  //              as a subscriber, NOT a lead ('all-leads' sequences do not reach them).
  // With neither, NOTHING renders. The hub's form posted to "#" when unconfigured — a signup that
  // silently went nowhere while the visitor believed they had subscribed.
  newsletter(s) {
    const external = /^https:\/\//i.test(String(s.action || '')) ? s.action : '';
    const native = !external && s._leadEndpoint ? s._leadEndpoint : '';
    if (!external && !native) return '';
    const hidden = native
      ? '\n      <input type="hidden" name="kind" value="newsletter" />'
        + '\n      <div class="hidden" aria-hidden="true"><label for="nl-website">Website</label><input id="nl-website" name="website" type="text" tabindex="-1" autocomplete="off" /></div>'
      : '';
    return `<section class="px-6 py-14${s._extraClass || ''}" aria-labelledby="nl-heading">
  <div class="mx-auto max-w-xl rounded-2xl border border-ink/10 bg-ink/5 px-6 py-8 text-center">
    <h2 id="nl-heading" class="text-2xl font-display font-bold text-ink">${esc(s.heading || 'Stay in touch')}</h2>
    ${s.blurb ? `<p class="mt-2 text-ink/70">${esc(s.blurb)}</p>` : ''}
${native ? '    <p id="newsletter-thanks" class="mt-4 hidden target:block rounded-lg border border-brand/40 bg-brand/10 px-4 py-3 text-ink">Thanks &mdash; you&rsquo;re subscribed. Every email includes a one-click unsubscribe link.</p>\n' : ''}    <form method="POST" action="${safeHref(external || native)}" class="mt-6 flex flex-col gap-3 sm:flex-row">
      <label class="sr-only" for="nl-email">Email address</label>
      <input id="nl-email" type="email" name="email" required maxlength="200" autocomplete="email" placeholder="you@example.com" class="flex-1 rounded-lg border border-ink/20 bg-paper px-3 py-2 text-ink" />${hidden}
      <button type="submit" class="rounded-lg bg-brand px-6 py-2 font-medium text-paper hover:opacity-90">${esc(s.button || 'Subscribe')}</button>
    </form>
  </div>
</section>`;
  },
  // Every tag with its count, largest first. Plain links — the tag pages themselves carry the lists.
  tagCloud(s) {
    const tags = Array.isArray(s.tags) ? s.tags : [];
    return `<section class="px-6 py-16${s._extraClass || ''}">
  <div class="mx-auto max-w-2xl">
    ${s.heading ? `<h1 class="text-3xl font-display font-bold text-ink">${esc(s.heading)}</h1>` : ''}
    ${tags.length ? `<ul class="ws-tags ws-tags-cloud mt-8">${tags.map((t) =>
    `<li><a href="${safeHref(t.href)}">${esc(t.label)}<span class="ws-tag-n">${Number(t.count) || 0}</span></a></li>`).join('')}</ul>`
    : '<p class="mt-6 text-ink/60">No tags yet.</p>'}
  </div>
</section>`;
  },
  // A video page. The YouTube player is loaded ONLY after the visitor clicks, from the
  // privacy-enhanced domain — until then the page makes no request to the player. The poster is
  // the entry's cover when it has one; without a cover it is YouTube's own thumbnail, which IS a
  // request to Google on page load. The hub's comment claimed "no third-party requests until
  // play"; that is only true when a cover is set, so set one if it matters.
  video(s) {
    // Re-validated here, not trusted from the model: a plan can reach this renderer by other routes
    // (a restored backup, a hand-edited state file), and these values land in attributes and URLs.
    const yt = /^[\w-]{11}$/.test(String(s.youtubeId || '')) ? s.youtubeId : '';
    const mp4 = /^https:\/\//i.test(String(s.videoUrl || '')) ? s.videoUrl : '';
    const poster = s.poster ? safeHref(s.poster) : '';
    const notes = s.html ? astroSafe(sanitizeHtml(s.html)) : '';
    let player = '';
    if (yt) {
      player = `    <div class="ws-player mt-8" data-yt="${esc(yt)}">
      <button type="button" aria-label="Play video: ${esc(s.heading || 'video')}">
        ${poster ? `<img src="${poster}" alt="" width="1280" height="720" />` : ''}
        <span class="ws-play" aria-hidden="true"></span>
      </button>
    </div>\n`;
    } else if (mp4) {
      player = `    <div class="ws-player mt-8"><video src="${safeHref(mp4)}" controls preload="metadata"${poster ? ` poster="${poster}"` : ''}></video></div>\n`;
    }
    // Plain string concatenation, NOT a template literal: this markup is itself built inside a JS
    // template literal, so any dollar-brace here would be interpolated by pipeline.js at build time
    // rather than by the browser.
    const script = yt ? `<script is:inline>
document.querySelectorAll('.ws-player[data-yt]').forEach(function (box) {
  ...
  btn.addEventListener('click', function () {
    ...
    return `<article class="px-6 py-16${s._extraClass || ''}">
  <div class="mx-auto max-w-3xl">
${s.eyebrow ? `    <p class="text-sm font-medium uppercase tracking-wide text-brand">${esc(s.eyebrow)}</p>\n` : ''}${s.heading ? `    <h1 class="mt-2 text-3xl sm:text-4xl font-display font-bold text-ink">${esc(s.heading)}</h1>\n` : ''}${s.standfirst ? `    <p class="mt-4 text-lg text-ink/70">${esc(s.standfirst)}</p>\n` : ''}${s.meta ? `    <p class="mt-3 text-sm text-ink/60">${esc(s.meta)}</p>\n` : ''}${player}${notes ? `    <div class="ws-article mt-8">\n${notes}\n    </div>\n` : ''}${tagChips(s.tags)}${sourceLine(s.source)}  </div>
</article>
${script}`;
  },
  article(s) {
    const body = astroSafe(sanitizeHtml(s.html || ''));
    const hasMeta = s.eyebrow || s.meta;
    return `<article class="px-6 py-16${s._extraClass || ''}">
  <div class="mx-auto max-w-2xl">
${s.eyebrow ? `    <p class="text-sm font-medium uppercase tracking-wide text-brand">${esc(s.eyebrow)}</p>\n` : ''}${s.heading ? `    <h1 class="mt-2 text-3xl sm:text-4xl font-display font-bold text-ink">${esc(s.heading)}</h1>\n` : ''}${s.standfirst ? `    <p class="mt-4 text-lg text-ink/70">${esc(s.standfirst)}</p>\n` : ''}${s.meta ? `    <p class="mt-3 text-sm text-ink/60">${esc(s.meta)}</p>\n` : ''}${hasMeta ? '    <hr class="mt-6 border-ink/10" />\n' : ''}${s.image ? `    <img src="${safeHref(s.image)}" alt="${esc(s.imageAlt || '')}" class="mt-8 w-full rounded-lg" loading="lazy" />\n` : ''}    <div class="ws-article mt-8">
${body}
    </div>
${tagChips(s.tags)}${sourceLine(s.source)}  </div>
</article>`;
...
// Tag chips under an entry or in a listing. Empty string when there are none, so an untagged entry
// renders byte-for-byte as it did before tags existed.
function tagChips(tags) {
  const list = Array.isArray(tags) ? tags.filter((t) => t && t.label && t.href) : [];
  if (!list.length) return '';
  return `    <ul class="ws-tags mt-6" aria-label="Tags">${list.map((t) =>
    `<li><a href="${safeHref(t.href)}">${esc(t.label)}</a></li>`).join('')}</ul>\n`;
}

// "Originally published at …" for curated or ingested material. The URL was validated as http(s)
// by the model and is re-checked here; off-site, so it opens in a new tab per the site convention.
function sourceLine(src) {
  if (!src || !/^https?:\/\//i.test(String(src.url || ''))) return '';
  let label = src.name;
  if (!label) { try { label = new URL(src.url).hostname.replace(/^www\./, ''); } catch { label = src.url; } }
  return `    <p class="ws-source mt-6">Originally published at <a href="${safeHref(src.url)}" rel="noopener noreferrer" target="_blank">${esc(label)}</a></p>\n`;
}

// Escape the two characters Astro treats as expression delimiters. Applied ONLY to already-
// sanitised markup destined for a .astro template; the entities render as literal braces.
function astroSafe(html) {
  return String(html == null ? '' : html).replace(/\{/g, '&#123;').replace(/\}/g, '&#125;');
}

// Typography for author-supplied article markup. The rich body carries NO classes (it is the
// author's HTML), and this project has no @tailwindcss/typography plugin, so descendant styling has
// to be real CSS. Emitted into <head> only when a plan actually uses an `article` section.
function articleProseStyle() {
  return `      .ws-article{color:var(--color-ink,#1a1a1a);line-height:1.75}
      .ws-article > * + *{margin-top:1.1em}
      .ws-article h2{font-size:1.6rem;font-weight:700;line-height:1.3;margin-top:2em}
      .ws-article h3{font-size:1.3rem;font-weight:700;line-height:1.35;margin-top:1.7em}
      .ws-article h4{font-size:1.1rem;font-weight:700;margin-top:1.5em}
      .ws-article p{opacity:.85}
      .ws-article a{color:var(--color-brand,#2563eb);text-decoration:underline}
      .ws-article ul,.ws-article ol{padding-left:1.5rem}
      .ws-article ul{list-style:disc}
      .ws-article ol{list-style:decimal}
      .ws-article li+li{margin-top:.35em}
      .ws-article blockquote{border-left:3px solid var(--color-brand,#2563eb);padding:.25rem 0 .25rem 1rem;font-style:italic;opacity:.9}
      .ws-article img{max-width:100%;height:auto;border-radius:.5rem}
      .ws-article table{width:100%;border-collapse:collapse;display:block;overflow-x:auto}
      .ws-article th,.ws-article td{border:1px solid rgba(0,0,0,.12);padding:.5rem .65rem;text-align:left}
      .ws-article pre{overflow-x:auto;padding:1rem;border-radius:.5rem;background:rgba(0,0,0,.05)}
      .ws-article code{font-size:.9em}
      .ws-article figure figcaption{font-size:.9rem;opacity:.7;margin-top:.5em}
      /* The site header is sticky on some templates; keep anchor jumps (footnotes, per-section
         links in long archival documents) from landing underneath it. */
      .ws-article [id]{scroll-margin-top:5rem}`;
}

// Does any page carry author-supplied prose? Drives the conditional prose stylesheet. A video's
// show notes are prose too, so a video-only site needs it as much as a blog does.
function planHasArticle(plan) {
  return (Array.isArray(plan && plan.pages) ? plan.pages : [])
    .some((p) => (Array.isArray(p && p.sections) ? p.sections : [])
      .some((s) => s && (s.type === 'article' || s.type === 'video')));
}

// Does any page actually USE a hub component — a player, a tag cloud, a tag chip, a video badge or
// a source line? Gated on use, not on section type: an ordinary blog with no tags, videos or
// attribution should not ship ~2 KB of player CSS it will never apply. Verified against the real
// Oregon and Truth Counters plans, where this keeps the layout unchanged apart from the feed link.
function planHasHub(plan) {
  const hasTags = (x) => Array.isArray(x && x.tags) && x.tags.length > 0;
  return (Array.isArray(plan && plan.pages) ? plan.pages : [])
    .some((p) => (Array.isArray(p && p.sections) ? p.sections : []).some((s) => {
      if (!s) return false;
      if (s.type === 'video' || s.type === 'tagCloud') return true;
      if (s.type === 'article') return hasTags(s) || !!s.source;
      if (s.type === 'articleList') return (Array.isArray(s.items) ? s.items : [])
        .some((it) => hasTags(it) || (it && it.kind === 'video'));
      return false;
    }));
}

// Styles for the hub components. GLOBAL for the same reason as the prose styles (29a7442): these
// classes are emitted by page components and by set:html-free markup the layout does not own, so a
// scoped block would compile to selectors carrying the layout's data-astro-cid and match nothing.
function hubStyle() {
  return `      .ws-tags{display:flex;flex-wrap:wrap;gap:.5rem;list-style:none;padding:0;margin-left:0}
      .ws-tags a{display:inline-flex;align-items:center;gap:.4rem;padding:.25rem .7rem;border-radius:999px;
        font-size:.8rem;text-decoration:none;color:inherit;border:1px solid rgba(0,0,0,.14)}
      .ws-tags a:hover{border-color:var(--color-brand,#2563eb);color:var(--color-brand,#2563eb)}
      .ws-tags-cloud a{font-size:.95rem;padding:.4rem .9rem}
      .ws-tag-n{font-size:.75rem;opacity:.6}
      .ws-badge{display:inline-block;margin-right:.5rem;padding:.05rem .45rem;border-radius:.25rem;
        background:var(--color-brand,#2563eb);color:#fff;letter-spacing:.04em}
      .ws-source{font-size:.9rem;opacity:.75}
      .ws-source a{text-decoration:underline}
      .ws-thumb-video{position:relative;display:block}
      .ws-thumb-video::after{content:"";position:absolute;left:50%;top:calc(50% - .5rem);width:3.2rem;height:3.2rem;
        transform:translate(-50%,-50%);border-radius:50%;background:rgba(0,0,0,.6)}
      .ws-player{position:relative;aspect-ratio:16/9;background:#000;border-radius:.75rem;overflow:hidden}
      .ws-player button{all:unset;cursor:pointer;display:block;width:100%;height:100%;position:relative}
      .ws-player button:focus-visible{outline:3px solid var(--color-brand,#2563eb);outline-offset:3px}
      .ws-player img,.ws-player iframe,.ws-player video{display:block;width:100%;height:100%;object-fit:cover;border:0}
      .ws-play{position:absolute;left:50%;top:50%;width:4.5rem;height:4.5rem;transform:translate(-50%,-50%);
        border-radius:50%;background:rgba(0,0,0,.7);transition:background .15s}
      .ws-play::after{content:"";position:absolute;left:54%;top:50%;transform:translate(-50%,-50%);
        border-style:solid;border-width:.8rem 0 .8rem 1.3rem;border-color:transparent transparent transparent #fff}
      .ws-player button:hover .ws-play{background:var(--color-brand,#2563eb)}`;
}
...
function renderBase(plan) {
  ...
  // The article prose styles MUST be global. Astro scopes a plain <style> by appending a
  // data-astro-cid attribute to both the selector and the layout's own elements:
  //     .ws-article[data-astro-cid-X] p[data-astro-cid-X] { ... }
  // The article body is injected with `set:html`, so none of that markup ever receives the
  // attribute — and neither does the wrapper, which is emitted by the PAGE component rather than
  // by this layout. Every one of the 19 prose rules therefore matched nothing, and two live sites
  // shipped with correct, present, entirely inert CSS. The existing test asserted only that
  // renderBase CONTAINS ".ws-article", which stayed true the whole time.
  const globalStyle = [planHasArticle(plan) ? articleProseStyle() : null,
    planHasHub(plan) ? hubStyle() : null].filter(Boolean).join('\n') || null;
  // Feed autodiscovery. Only when the feed will actually exist — rssXml needs the domain and at
  // least one published entry — so no page ever advertises a feed that 404s.
  const feedLink = planHasFeed(plan)
    ? `    <link rel="alternate" type="application/rss+xml" title="${astroSafe(esc(plan.siteName || 'Feed'))}" href="/rss.xml" />\n`
    : '';
...
${feedLink}${headStyle ? `    <style>\n${headStyle}\n    </style>\n` : ''}${globalStyle ? `    <style is:global>\n${globalStyle}\n    </style>\n` : ''}    <slot name="head" />
  </head>
...
function renderPage(page, meta, plan = {}) {
  ...
  // page.extraLd carries schema a generator attached to the page itself — currently the Article
  // entity from an expanded plan.articles entry (lib/web-studio/articles.js).
  const attached = Array.isArray(page.extraLd) ? page.extraLd.filter((x) => x && typeof x === 'object') : [];
  const extraLd = [...actionLd, ...(faqLd ? [faqLd] : []), ...attached];
  const ldPayload = extraLd.length ? [pageLd, ...extraLd] : pageLd;
  const canonical = aeoEmit.canonicalUrl(plan, page);
  // The layout import must climb out of however many directories the page sits in. This was
  // hardcoded to '../', which is correct ONLY for a page directly in src/pages — so every NESTED
  // page emitted an unresolvable import and the whole Astro build died:
  //   Could not resolve "../layouts/Base.astro" from "src/pages/article/<slug>.astro"
  // That is a pre-existing defect, not one the article feature introduced: plan.dynamic has always
  // generated `${pathPrefix}/${slug}` pages with the same shape. Articles are simply the first
  // generator to be run through a real build. Found by the FIRST successful worker execution —
  // no amount of string-level testing would have surfaced it, because nothing ever resolved it.
  const layoutDepth = pageFile(page.path).split(/[\\/]/).length - 3;   // src/pages/<...>/file.astro
  const baseImport = '../'.repeat(Math.max(0, layoutDepth) + 1) + 'layouts/Base.astro';
  // title/description go in as Astro EXPRESSIONS `{"..."}`, not quoted attributes. JSON.stringify
  // escapes a quote as \" — which HTML attribute parsing does not honour, so `title="Say \"hi\""`
  // terminated the attribute early and left `hi\"` as stray junk. Backslash is not an escape
  // character in HTML. As an expression the value is JavaScript, where \" is correct, and Astro
  // escapes it on render. (`og={...}` next to it was already expression form.)
  return `---
...
function renderPlanToWorkspace(workspaceDir, plan, meta = {}) {
  // Both expansions run BEFORE anything else, so every downstream emitter (sitemap, llms.txt,
  // JSON-LD, OKF bundle, WCAG gate) sees article and dataset pages exactly like hand-written ones.
  plan = expandArticlePages(expandDynamicPages(plan));
  ...
  // RSS for the hub feed. Links go through canonicalUrl so they carry the same trailing slash as
  // the canonical tags and the sitemap. Removed when there is no feed, so a site that deletes its
  // last entry does not keep serving a stale one from an earlier render.
  const rss = rssXml(plan, (page) => aeoEmit.canonicalUrl(plan, page));
  const rssFile = path.join(pub, 'rss.xml');
  if (rss) fs.writeFileSync(rssFile, rss);
  else if (fs.existsSync(rssFile)) fs.rmSync(rssFile);
...
function writeProvenanceSidecar(workspaceDir, distDir, plan, provenanceMeta, signProvenance) {
  ...
  // A null credential means the instance has no signing key right now (missing after a restore,
  // or never initialised). Write nothing rather than an unsigned sidecar: an unsigned file at the
  // well-known path would read as a broken credential, not as an absent one.
...
async function createSiteFromBrief(opts, deps) {
  ...
  // 1. Plan (web-studio-lead). Give it a wide token budget: Opus 5 runs this agent at
...
module.exports = { createSiteFromBrief, renderPlanToWorkspace, writeProvenanceSidecar, extractJson, renderPage, renderBase, renderSection, buildSiteKnowledge, applyFunnel, expandDynamicPages, astroSafe, planHasArticle };
