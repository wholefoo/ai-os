...
// Framework configs that mark a directory as application SOURCE (it must be built before it is a
// website). Shared by the shell detector and by diagnoseNoIndex.
const FRAMEWORK_CONFIGS = ['vite.config.ts', 'vite.config.js', 'vite.config.mjs', 'next.config.js', 'next.config.mjs', 'next.config.ts', 'astro.config.mjs', 'astro.config.ts', 'nuxt.config.ts', 'svelte.config.js', 'gatsby-config.js', 'angular.json', 'remix.config.js'];
// A script entry that only a build step can turn into something a browser runs.
const SOURCE_ENTRY_EXT = /\.(tsx?|mts|cts|jsx|vue|svelte)(?:[?#]|$)/i;

/**
 * Is <root>/index.html an UNBUILT app shell rather than a website?
 *
 * THE INCIDENT (2026-09-10): a Vite + React repo imported from GitHub published a BLANK page. Vite
 * keeps a SOURCE index.html at the repo root whose only content is
 * `<script type="module" src="/src/main.tsx">`. .tsx is not a static asset, so main.tsx was dropped
 * on import and the page loaded a file that did not exist. The "build it first" message never fired
 * because it only ran when there was NO index.html, and a Vite repo always has one.
 *
 * Three signals, each specific enough not to catch a hand-written site:
 *   - a <script src> ending in a source extension (.ts/.tsx/.jsx/.vue/.svelte) — never valid in a
 *     browser as-is;
 *   - Create React App's template placeholder %PUBLIC_URL%, which only its build substitutes;
 *   - a /src/ script entry sitting beside package.json AND a framework config. A plain static site
 *     with /src/app.js and no package.json is NOT flagged — that is a real site.
 */
function isUnbuiltAppShell(root) {
  let html; try { html = fs.readFileSync(path.join(root, 'index.html'), 'utf8'); } catch { return false; }
  if (/%PUBLIC_URL%/.test(html)) return true;
  const srcs = [...html.matchAll(/<script\b[^>]*?\bsrc\s*=\s*["']([^"']+)["']/gi)].map((m) => m[1]);
  if (srcs.some((u) => SOURCE_ENTRY_EXT.test(u))) return true;
  if (!srcs.some((u) => /^\/?src\//i.test(u))) return false;
  let names; try { names = new Set(fs.readdirSync(root)); } catch { return false; }
  return names.has('package.json') && FRAMEWORK_CONFIGS.some((c) => names.has(c));
}

// A directory is a usable site root only if it has an index.html that is NOT an unbuilt shell.
const isSiteRoot = (p) => fs.existsSync(path.join(p, 'index.html')) && !isUnbuiltAppShell(p);

/**
 * Pick the folder to publish. Returns a directory; if every index.html found is an unbuilt shell,
 * returns that shell directory so the caller can REFUSE with an actionable reason (it must never
 * be published — see isUnbuiltAppShell).
 *
 * The repo root is still checked FIRST, so a real static site with a stale dist/ beside it keeps
 * winning. What changed is that a root holding only an unbuilt shell no longer wins by default: a
 * committed dist/, dist/public (Replit full-stack template), build/ etc. is preferred over it.
 */
function findStaticRoot(dir) {
  const candidates = ['', 'dist', 'dist/public', 'build', 'out', 'public', '_site', 'site', 'www', 'static', 'htdocs', 'web', 'wwwroot', 'docs', 'app', 'client/dist'];
  let shellRoot = null;
  const tryCandidates = (base) => {
    for (const c of candidates) {
      const p = c ? path.join(base, c) : base;
      if (isSiteRoot(p)) return p;
      if (!shellRoot && fs.existsSync(path.join(p, 'index.html'))) shellRoot = p;
    }
    return null;
  };
  const direct = tryCandidates(dir);
  if (direct) return direct;
  // single top-level dir (a downloaded zip) -> descend and re-check the candidates inside it
  try {
    const items = fs.readdirSync(dir, { withFileTypes: true }).filter((d) => !d.name.startsWith('.'));
    if (items.length === 1 && items[0].isDirectory()) {
      const inner = tryCandidates(path.join(dir, items[0].name));
      if (inner) return inner;
    }
  } catch { /* fall through */ }
  // last resort: shallow search (depth <= 3) for the SHALLOWEST real index.html anywhere, skipping
  // dot-dirs and node_modules. A shell is not a match, so the search keeps descending past it.
  ...
  const seek = (cur, depth) => {
    ...
    if (items.some((it) => it.isFile() && it.name === 'index.html')) {
      if (!isUnbuiltAppShell(cur)) { if (depth < bestDepth) { best = cur; bestDepth = depth; } return; }
      if (!shellRoot) shellRoot = cur;
    }
...
function diagnoseNoIndex(dir) {
  ...
  const CONFIGS = FRAMEWORK_CONFIGS;
...
async function _importToWorkspace({ workspaceDir, zipBuffer, githubUrl, githubToken }) {
  ...
  try {
    ...
    // Only an unbuilt app shell was found. Ingesting it would publish a BLANK page (the 2026-09-10
    // incident). Copy nothing, so staticBuild fails and the site shows this reason as build_failed.
    if (isUnbuiltAppShell(root)) {
      const html = (() => { try { return fs.readFileSync(path.join(root, 'index.html'), 'utf8'); } catch { return ''; } })();
      const entry = ([...html.matchAll(/<script\b[^>]*?\bsrc\s*=\s*["']([^"']+)["']/gi)].map((m) => m[1]).find((u) => SOURCE_ENTRY_EXT.test(u) || /^\/?src\//i.test(u))) || null;
      const reason = `This is the SOURCE of an app, not a built website: its index.html ${entry ? `loads ${entry}, which` : 'is a template that'} has to be compiled first, and Web Studio never runs a build. Run the build (usually "npm run build") and either commit the output folder to the repo (a dist/ or build/ folder is picked up automatically) or import that folder as a ZIP.`;
      return { ok: false, hasIndex: false, reason, unbuilt: true, written: [], warnings: [], bytes: 0, count: 0 };
    }
...
module.exports = { importToWorkspace, sanitizeRelPath, extractZip, ingestDir, findStaticRoot, isUnbuiltAppShell, cloneGitHub, SAFE_EXT, MAX_TOTAL_BYTES, MAX_FILES };
