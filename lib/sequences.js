...
//   sequence   = { id, name, trigger: 'site-lead'|'free-audit'|'booking'|'site-newsletter'|'all-leads', siteId|null, enabled,
...
// Every source a sequence can be triggered by. 'site-newsletter' is a signup from a Web Studio site's
// newsletter form (the hub integration).
const TRIGGERS = ['site-lead', 'free-audit', 'booking', 'site-newsletter', 'all-leads'];
// Sources that are NOT leads: the person consented to something other than being contacted, so
// 'all-leads' does not reach them. See enroll().
const NOT_A_LEAD = new Set(['site-newsletter']);
...
function validateSequence(seq) {
  ...
  if (!TRIGGERS.includes(seq.trigger)) errs.push('trigger must be one of: ' + TRIGGERS.join(', '));
...
function enroll({ email, name = '', siteId = null, source }, { sequences, enrollments, suppression }, now = Date.now()) {
  ...
  for (const seq of sequences) {
    ...
    // 'all-leads' means everyone who ASKED TO BE CONTACTED — a form enquiry, an audit, a booking.
    // A newsletter subscriber asked for a newsletter, not a sales conversation, so they match only
    // a sequence built for them (trigger 'site-newsletter'). Without this exclusion, adding the
    // newsletter signup would have enrolled every subscriber into the operator's nurture sequences.
    const triggerMatch = seq.trigger === source
      || (seq.trigger === 'all-leads' && !NOT_A_LEAD.has(source));
...
async function tick({ sequences, enrollments, suppression }, deps, now = Date.now()) {
  ...
  for (const en of enrollments) {
    ...
    if (r && r.sent) {
      if (en.step === dispatchedStep) advance(en, seq, now);
      out.sent++;
